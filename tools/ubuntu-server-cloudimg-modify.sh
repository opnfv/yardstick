...
if [ $YARD_IMG_ARCH = "arm64" ]; then
    ...
    sed -i -e 's/OPTON += -march=native -mtune=native/OPTON += -march=armv8-a -mtune=generic/g' -e 's/OPTON += -march=native -mmacosx-version-min=10.10/OPTON += -march=armv8-a -mmacosx-version-min=10.10/g' /opt/tempT/UnixBench/Makefile
else
