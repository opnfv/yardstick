...
====================================================
pci@0000:03:00.0  eth0        network    Ethernet Controller
pci@0000:0b:00.0  eth1        network    Ethernet Controller
"""
...
class TestStandaloneContext(StandaloneContextTestCase):
    ...
    def setUp(self):
        ...
        self.test_context.nodes = deepcopy(NODES_DATA)
        self.test_context.node = self.test_context.nodes[0]
    ...
    def test_get_nic_details(self, mock_ssh, *_):
        mock_ssh.execute.return_value = 0, LSHW_GREP_OUTPUT, ''
        self.test_context._ssh_helper = mock_ssh

        self.assertIsNone(self.test_context.nic_details)
        self.assertIsNotNone(self.test_context.get_nic_details())
        self.assertIsNotNone(self.test_context.nic_details)
        ssh_call_count = mock_ssh.execute.call_count
        self.assertIsNotNone(self.test_context.get_nic_details())
        self.assertEqual(mock_ssh.execute.call_count, ssh_call_count)

    def test__install_required_libraries_not_first_run(self, *_):
        self.test_context.first_run = False
        self.test_context._ssh_helper = mock.Mock()
        self.test_context._install_required_libraries()

    def test__setup_context(self, *_):
        with self.assertRaises(NotImplementedError):
            self.test_context._setup_context()

    @mock.patch("yardstick.benchmark.contexts.standalone.VnfSshHelper")
    def test__make_vm_template(self, mock_ssh, *_):
        attrs = self.make_attrs('sriov')

        mock_ssh_instance = mock.Mock()
        mock_ssh_instance.execute.return_value = 0, 'vm1', ''
    ...
    @mock.patch("yardstick.ssh.SSH")
    def test__start_vm_already_started(self, mock_ssh, *_):
        attrs = self.make_attrs('sriov')
        ...
        self.assertIsNone(self.test_context._start_vm('remote/cfg/path/data.xml'))

    @mock.patch("yardstick.ssh.SSH")
    def test__start_vm_create_fail(self, mock_ssh, *_):
        attrs = self.make_attrs('sriov')
        ...
        with self.assertRaises(RuntimeError) as raised:
            self.test_context._start_vm('remote/cfg/path/data.xml')
