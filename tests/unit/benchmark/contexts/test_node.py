...
class NodeContextTestCase(unittest.TestCase):
    ...
    def test_init_negative(self, mock_path_join):
        ...
        self.assertEqual(mock_path_join.call_count, join_calls)
    ...
    def test_get_server(self):
        with self.assertRaises(ValueError):
            self.test_context.get_server('my.vnf1')

        self.test_context.name = 'vnf1'
        self.test_context.nodes = [{'name': 'my', 'value': 100}]
        expected = {'name': 'my.vnf', 'value': 100}
        result = self.test_context.get_server('my.vnf')
        self.assertDictEqual(result, expected)

    def test_get_context_from_server(self):
        with self.assertRaises(ValueError):
            self.test_context.get_context_from_server('my.vnf1')
