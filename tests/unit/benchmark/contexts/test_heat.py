...
class HeatContextTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.name_iter = ('vnf{}'.format(x) for x in count(0, step=3))
...
#        mock_template.add_network.assert_called_with("bar-fool-network", 'physnet1', None)
    ...
    def test_get_server(self):
        bad_name = next(self.name_iter)
        self.test_context.name = good_name = next(self.name_iter)
        self.test_context.stack = mock.MagicMock()
        self.test_context.stack.outputs = {}
        self.test_context._user = 'that_guy'

        with self.assertRaises(ValueError):
            self.test_context.get_server({'name': bad_name})

        self.assertIsNotNone(self.test_context.stack)

        result = self.test_context.get_server({'name': good_name})
        self.assertEqual(result['user'], 'that_guy')
        self.assertIsNone(result['private_ip'])
        self.assertIsNotNone(result['key_filename'])
    ...
    def test_get_context_from_server(self):
        ...
        with self.assertRaises(ValueError):
            self.test_context.get_context_from_server(bad_name)

        result = self.test_context.get_context_from_server(good_name)
