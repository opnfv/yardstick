...
class HeatContextTestCase(unittest.TestCase):
    ...
    def wrapUp(self):
        if not isinstance(self.test_context.stack, heat.HeatTemplate):
            self.test_context.stack = None
...
#        mock_template.add_network.assert_called_with("bar-fool-network", 'physnet1', None)
    ...
    @staticmethod
    def mock_stack(context):
        try:
            return context.stack.outputs
    ...
    def test_get_server(self):
        ...
        with self.assertRaises(ValueError):
            self.test_context.get_server({'name': 'my.vnf2'})

        for context in base.Context.list:
            self.mock_context(context)

        result = self.test_context.get_server({'name': 'my.vnf1'})
        self.assertEqual(result['user'], 'my')
        self.assertIsNone(result['private_ip'])
        self.assertIsNotNone(result['key_filename'])
