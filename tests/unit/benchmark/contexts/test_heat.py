...
class HeatContextTestCase(unittest.TestCase):
    ...
    def test__add_resources_to_template_no_servers(self, mock_template):
        ...
        mock_template.add_router.assert_called_with("bar-fool-network-router", netattrs["external_network"], "bar-fool-network-subnet", netattrs["provider"])
        mock_template.add_router_interface.assert_called_with("bar-fool-network-router-if0", "bar-fool-network-router", "bar-fool-network-subnet", netattrs["provider"])
