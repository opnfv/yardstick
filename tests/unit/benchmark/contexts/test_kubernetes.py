...
class KubernetesTestCase(unittest.TestCase):
    ...
    @mock.patch('{}.yaml'.format(prefix))
    @mock.patch('__builtin__.open')
    def test_get_ip(self, mock_open, mock_yaml):
        attrs = {
            'clusters': [
                {
                    'cluster': {
                        'server': 'http://172.16.10.131:30000'
                    }
                }
            ]
        }

        mock_yaml.safe_load.return_value = attrs
        k8s_context = KubernetesContext()
        k8s_context.init(context_cfg)
        ip = k8s_context._get_ip()
        self.assertEqual(ip, '172.16.10.131')
