...
class TestCloudInit(unittest.TestCase):
    ...
    def test_generate_iso_images(self, mock_ansible_common, _):
        ...
        mock_ansible_exec = mock.Mock()
        mock_ansible_common.return_value = mock_ansible_exec
        mock_ansible_exec.execute_ansible = mock.Mock()

        cloud_init = CloudInit(node)
        cloud_init.generate_iso_images()
        mock_ansible_exec.execute_ansible.assert_not_called()

        node['cloud_init']['generate_iso_images'] = True
        cloud_init = CloudInit(node)
        cloud_init.generate_iso_images()
        mock_ansible_exec.execute_ansible.assert_called()
    ...
    def test_iso_image_path(self):
        ...
        self.assertEqual('/var/lib/yardstick', cloud_init.iso_image_path)
