...
class StandaloneModelTestCase(unittest.TestCase):
    ...
    def test_pin_vcpu_for_perf(self, mock_update_interrupts_hugepages_perf, mock_CpuSysCores):
        ssh_mock = mock.Mock(autospec=ssh.SSH)
        ssh_mock.execute.return_value = 0, "a", ""
        status = model.pin_vcpu_for_perf(ssh_mock, "vm_0", 4)
    ...
    def test_install_req_libs(self):
        ...
        model.install_req_libs(ssh_mock, extra_pgs=['pkg1', 'pkg2'])
    ...
    def test_wait_for_vnfs_to_start(self, mock_find_mgmt_ip):
...
class OvsDeployTestCase(unittest.TestCase):
    ...
    def test_prerequisite(self, mock_os):
        model.install_req_libs = mock.Mock()
        self.ovs_deploy.prerequisite()

    @mock.patch('yardstick.benchmark.contexts.standalone.model.os')
    def test_ovs_deploy(self, mock_os):
    ...
    @mock.patch('yardstick.benchmark.contexts.standalone.model.os')
    def test_ovs_deploy_with_failure(self, mock_os):
