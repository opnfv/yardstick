...
class ModelLibvirtTestCase(unittest.TestCase):
    ...
    def test_add_sriov_interfaces(self):
       ...
       #  status, _, _ = connection.execute('ll %s' % base_image)
       #  if status:
    ...
    def test_create_snapshot_qemu_no_image_remote(self,
            mock_isfile, mock_normpath, mock_basename):
        ...
        mock_isfile.assert_called_once_with(base_image)
        mock_normpath.assert_called_once_with(base_image)
        mock_basename.assert_called_once_with(base_image)
        self.mock_ssh.put_file.assert_called_once_with(base_image,
                                                       'tmp/base_image')

    @mock.patch.object(os.path, 'isfile', return_value=False)
    def test_create_snapshot_qemu_no_image_local(self, mock_isfile):
