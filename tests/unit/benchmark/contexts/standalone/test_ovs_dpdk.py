...
class OvsDpdkContextTestCase(unittest.TestCase):
    ...
    def test___init__(self, mock_helper, mock_libvirt, mock_server):
        ...
        self.ovs_dpdk.libvirt = mock_helper
    ...
    def test_check_ovs_dpdk_env(self):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ssh_mock = mock.Mock(autospec=ssh.SSH)
            ssh_mock.execute = \
                mock.Mock(return_value=(0, "a", ""))
    ...
    def test_undeploy(self, mock_ssh):
        ...
        self.ovs_dpdk.libvirt.check_if_vm_exists_and_delete = mock.Mock()
        self.ovs_dpdk.cleanup_ovs_dpdk_env = mock.Mock()
    ...
    def test__enable_interfaces(self):
        ...
        self.ovs_dpdk.drivers = []
        self.ovs_dpdk.networks = self.NETWORKS
        self.ovs_dpdk.libvirt.add_ovs_interface = mock.Mock(return_value="")
        self.ovs_dpdk.get_vf_datas = mock.Mock(return_value="")
        self.assertIsNone(self.ovs_dpdk._enable_interfaces(0, ["private_0"], 'test'))

    @mock.patch('yardstick.benchmark.contexts.standalone.model.Server')
    def test_setup_ovs_dpdk_context(self, mock_server):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ssh_mock = mock.Mock(autospec=ssh.SSH)
            ssh_mock.execute = \
                mock.Mock(return_value=(0, "a", ""))
        ...
        self.ovs_dpdk.libvirt.check_if_vm_exists_and_delete = mock.Mock(return_value="")
        self.ovs_dpdk.libvirt.build_vm_xml = mock.Mock(return_value=[6, "00:00:00:00:00:01"])
        self.ovs_dpdk._enable_interfaces = mock.Mock(return_value="")
        self.ovs_dpdk.libvirt.virsh_create_vm = mock.Mock(return_value="")
        self.ovs_dpdk.libvirt.pin_vcpu_for_perf= mock.Mock(return_value="")
