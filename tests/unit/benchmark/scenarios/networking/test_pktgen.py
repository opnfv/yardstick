...
class PktgenTestCase(unittest.TestCase):
    ...
    def test_pktgen_run_with_ovs_dpdk(self, mock_ssh):
        args = {
            'options': {'packetsize': 60, 'number_of_ports': 10, 'duration': 20, 'ovs_dpdk': True},
            'sla': {'max_ppm': 1}
        }
        result = {}
        ...
        mock_ssh.SSH().execute.assert_called_with(
            "sudo bash pktgen.sh 172.16.0.138 10 60 20 1 1000000")
