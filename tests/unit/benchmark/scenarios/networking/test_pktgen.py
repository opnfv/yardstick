...
class PktgenTestCase(unittest.TestCase):
    ...
    def test_pktgen_get_vnic_type(self, mock_ssh):
        args = {
            'options': {'packetsize': 60},
        }
        p = pktgen.Pktgen(args, self.ctx)
        ...
        mock_ssh.SSH().execute.return_value = (0, 'ixgbevf', '')

        result = p._get_vnic_type()

        self.assertEqual(result, 'ixgbevf')
    ...
    def test_pktgen_get_sriov_queue_number(self, mock_ssh):
        ...
        p._get_sriov_queue_number()

        mock_ssh.SSH().execute.assert_called_with(
            "grep eth0-TxRx- /proc/interrupts | wc -l")
    ...
    def test_pktgen_get_available_queue_number(self, mock_ssh):
        ...
        mock_ssh.SSH().execute.return_value = (0, '4', '')
    ...
    def test_pktgen_setup_irqmapping_ovs(self, mock_ssh):
        ...
        mock_ssh.SSH().execute.return_value = (0, '10', '')

        p._setup_irqmapping_ovs(4)

        mock_ssh.SSH().execute.assert_called_with(
            "echo $((1<<3)) | sudo tee /proc/irq/10/smp_affinity")
    ...
    def test_pktgen_setup_irqmapping_sriov(self, mock_ssh):
        ...
        mock_ssh.SSH().execute.assert_called_with(
            "echo $((1<<1)) | sudo tee /proc/irq/10/smp_affinity")
    ...
    def test_pktgen_no_multiqueue_setup(self, mock_ssh):
        args = {
            'options': {'packetsize': 60, 'multiqueue': False},
        }
        p = pktgen.Pktgen(args, self.ctx)
        p.server = mock_ssh.SSH()
        p.client = mock_ssh.SSH()
    ...
    def test_pktgen_multiqueue_setup_sriov(self, mock_ssh):
        ...
        mock_result.return_value = "ixgbevf" 
        p._get_vnic_type = mock_result

        p.multiqueue_setup()

        self.assertEqual(p.queue_number, 2)

    def test_pktgen_run_with_pps(self, mock_ssh):
        args = {
            'options': {'packetsize': 60, 'number_of_ports': 10, 'duration': 20, 'pps': 2000000},
            'sla': {'max_ppm': 1}
        }
        result = {}
        p = pktgen.Pktgen(args, self.ctx)
        ...
        mock_ssh.SSH().execute.assert_called_with(
            "sudo bash pktgen.sh 172.16.0.138 10 60 20 1 2000000")

    def test_pktgen_run_with_multiqueque(self, mock_ssh):
        args = {
        ...
        mock_result2.return_value = 4
        p._get_available_queue_number = mock_result2
        p._enable_ovs_multiqueue  = mock_result2

        mock_result3 = mock.Mock()
        p._setup_irqmapping_ovs = mock_result3
        ...
        mock_ssh.SSH().execute.assert_called_with(
            "sudo bash pktgen.sh 172.16.0.138 10 60 20 2 1000000")
