...
from yardstick.benchmark.scenarios.networking.vnf_generic import \
    NetworkServiceTestCase, IncorrectConfig, open_relative_file
...
class TestNetworkServiceTestCase(unittest.TestCase):
    ...
    def test_map_topology_to_infrastructure(self):
        with mock.patch("yardstick.network_services.helpers.dpdkbindnic_helper.VnfSshHelper") as ssh:
    ...
    def test_map_topology_to_infrastructure_config_invalid(self):
        ...
        with mock.patch.dict(self.s.context_cfg, cfg):
            with mock.patch("yardstick.network_services.vnf_generic.vnf.vnf_ssh_helper") as ssh:
                ssh_mock = mock.Mock(autospec=ssh.SSH)
                ssh_mock.execute = \
                    mock.Mock(return_value=(0, SYS_CLASS_NET + IP_ADDR_SHOW, ""))
                ssh.from_node.return_value = ssh_mock
