...
def mocked_requests_config_post(*args, **kwargs):
    ...
    return MockResponseConfigPost('{}', 200)
...
def mocked_requests_config_get(*args, **kwargs):
    ...
    return MockResponseConfigGet('{"stack_created": "true"}', 200)
...
def mocked_requests_job_get(*args, **kwargs):
    ...
    return MockResponseJobGet('{"averages": {"_ssd_preconditioning.queue-depth.8.block-size.16384.duration": 6}}', 200)
...
def mocked_requests_delete_failed(*args, **kwargs):
    ...
    if args[0] == "http://StorPerf:5000/api/v1.0/configurations":
        ...
        return MockResponseDeleteFailed('{"message": "Job terminate failed"}',
                                        400)
...
class StorPerfTestCase(unittest.TestCase):
    ...
    def test_successful_run(self, mock_post, mock_get):
        options = {
            "agent_count": 8,
            "public_network": 'ext-net',
            "volume_size": 10,
        ...
        sample_output = '{"averages": {"_ssd_preconditioning.queue-depth.8.block-size.16384.duration": 6}}'

        expected_result = json.loads(sample_output)

        s.run(self.result)

        self.assertEqual(self.result, expected_result["averages"])
