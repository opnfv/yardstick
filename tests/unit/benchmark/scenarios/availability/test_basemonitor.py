...
class MonitorMgrTestCase(unittest.TestCase):
    ...
    def test__MonitorMgr_setup_successful(self, mock_monitor):
        instance = basemonitor.MonitorMgr({"process_num": 10})
    ...
    def test_MonitorMgr_getitem(self, mock_monitor):
        monitorMgr = basemonitor.MonitorMgr({"process_num": 10})
...
class BaseMonitorTestCase(unittest.TestCase):
    ...
    def test__basemonitor_start_wait_successful(self):
        ins = basemonitor.BaseMonitor(self.monitor_cfg, None, {"process_num": 10})
        ins.start_monitor()
        ins.wait_monitor()

    def test__basemonitor_all_successful(self):
        ins = self.MonitorSimple(self.monitor_cfg, None, {"process_num": 10})
        ins.setup()
        ins.run()
        ins.verify_SLA()
