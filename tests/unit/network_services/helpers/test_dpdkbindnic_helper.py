...
class TestDpdkNode(unittest.TestCase):
    ...
    def test_probe_dpdk_drivers(self):
        ...
        dpdk_node = DpdkNode('key', {'interfaces': interfaces})
        ...
        dpdk_node.probe_dpdk_drivers()
        self.assertNotIn('driver', interfaces['name1'])
        self.assertEqual(interfaces['name2']['driver'], 'driver2')
        self.assertEqual(interfaces['name3']['driver'], 'driver1')
    ...
    def test_check(self):
        ...
        interface = {'local_mac': 404}

        dpdk_node = DpdkNode('key', {'interfaces': {'if1': interface}})
        dpdk_node._ssh_helper = mock_ssh_helper
        dpdk_node.probe_missing_values = mock_probe_missing = mock.Mock(side_effect=update)
        dpdk_node.force_rebind = mock_force_rebind = mock.Mock()
    ...
    def test_probe_devices(self, mock_ssh):
        mock_instance = mock_ssh()
        mock_instance.execute.return_value = 0, '', ''

        dpdk_node = DpdkNode('key', {"interfaces": {}})

        dpdk_node.probe_ip_tool()
