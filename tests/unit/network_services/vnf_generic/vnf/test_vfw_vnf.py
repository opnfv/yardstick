...
class TestFWApproxVnf(unittest.TestCase):
    ...
    def test_run_vfw(self, MultiPortConfig, hex, evali, mock_open):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ssh_mock = mock.Mock(autospec=ssh.SSH)
            ssh_mock.execute = \
                mock.Mock(return_value=(0, "", ""))
    ...
    @mock.patch('yardstick.network_services.vnf_generic.vnf.vfw_vnf.CpuSysCores')
    def test__validate_cpu_cfg(self, CpuSysCores):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ...
            self.scenario_cfg['vnf_options'] = {'acl': {'cfg': "",
                                                        'rules': ""}}
            vfw_approx_vnf._run_acl = mock.Mock(return_value=0)
            ...
            vfw_approx_vnf.resource.check_if_sa_running(return_value=[1])
            vfw_approx_vnf.resource.amqp_collect_nfvi_kpi(return_value={})
            vfw_approx_vnf.generate_port_pairs = mock.Mock()
            vfw_approx_vnf.tg_port_pairs = [[[0], [1]]]
            vfw_approx_vnf.topology = ""
            vnf_cfg = {'lb_config': 'SW', 'worker_threads': 2}
            CpuSysCores.get_core_socket = mock.Mock(return_value=[{'0': ['1', '2']}])
            vfw_approx_vnf._get_cpu_sibling_list = mock.Mock(return_value=1)
            self.assertEqual(1, vfw_approx_vnf._validate_cpu_cfg(vnf_cfg))
    ...
    def test__collect_resource_kpi(self):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ...
            vfw_approx_vnf.resource.check_if_sa_running(return_value=[1])
            vfw_approx_vnf.resource.amqp_collect_nfvi_kpi(return_value={})
            self.assertIsNotNone(vfw_approx_vnf._collect_resource_kpi())
    ...
    def test_instantiate(self, Context):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ...
            vfw_approx_vnf._validate_cpu_cfg = \
    ...
    def test_instantiate_panic(self, Context):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ...
            vfw_approx_vnf.q_out.put("PANIC")
            vfw_vnf.WAIT_TIME = 3
            self.assertRaises(RuntimeError, vfw_approx_vnf.instantiate,
                              self.scenario_cfg, self.context_cfg)
    ...
    def test__get_ports_gateway(self):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ...
            self.assertEqual('152.16.100.20',
                             vfw_approx_vnf._get_ports_gateway('xe0'))
    ...
    def test__get_cpu_sibling_list(self):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ...
            self.assertEqual([], vfw_approx_vnf._get_cpu_sibling_list(cores))
            cores = ['1']
            vfw_approx_vnf.connection = ssh_mock
            self.assertEqual(['1', '2'], vfw_approx_vnf._get_cpu_sibling_list(cores))
