...
class TestFWApproxVnf(unittest.TestCase):
    ...
    def test_collect_kpi(self, mock_time, mock_process):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ...
            vfw_approx_vnf.vnf_execute = mock.Mock(return_value="")
    ...
    def test_get_stats(self, mock_process):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ...
            vfw_approx_vnf.vnf_execute = \
                mock.Mock(return_value=mock_result)
            self.assertEqual(mock_result, vfw_approx_vnf.get_stats())
    ...
    def test_instantiate_panic(self, mock_process):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ...
            vfw_approx_vnf.WAIT_TIME = 1
            vfw_approx_vnf.resource_helper = mock.MagicMock()
            vfw_approx_vnf._build_config = mock.MagicMock()
            vfw_approx_vnf._vnf_process = mock.MagicMock()
            vfw_approx_vnf._vnf_process.start = mock.Mock()
            vfw_approx_vnf._vnf_process.is_alive = mock.Mock(return_value=True)
            self.assertRaises(ValueError, vfw_approx_vnf.instantiate,
                              self.scenario_cfg, self.context_cfg)
            vfw_approx_vnf.q_out.put("PANIC")
            vfw_approx_vnf.WAIT_TIME = 3
            self.assertRaises(ValueError, vfw_approx_vnf.instantiate,
                              self.scenario_cfg, self.context_cfg)
