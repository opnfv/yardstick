...
class MockSTLStateError(BaseException):
    pass
...
class TestClientResourceHelper(unittest.TestCase):
    ...
    @mock.patch('yardstick.network_services.vnf_generic.vnf.sample_vnf.STLStateError',
                new_callable=lambda: MockSTLStateError)
    def test_get_stats_not_connected(self, mock_state_error):
        # TODO: fix or skip for PY27, mocked exception fails in PY27
        vnfd_helper = VnfdHelper({})
        ssh_helper = mock.Mock()
        scenario_helper = mock.Mock()
        dpdk_setup_helper = DpdkVnfSetupEnvHelper(vnfd_helper, ssh_helper, scenario_helper)
        ...
        with mock.patch('yardstick.network_services.vnf_generic.vnf.sample_vnf.LOG'):
