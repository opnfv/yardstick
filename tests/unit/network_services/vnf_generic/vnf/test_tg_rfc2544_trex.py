...
class TestTrexTrafficGenRFC(unittest.TestCase):
    ...
    @mock.patch(SSH_HELPER)
    def old_test__get_rfc_tolerance(self, ssh):
        mock_ssh(ssh)
        trex_traffic_gen = TrexTrafficGenRFC('vnf1', self.VNFD_0)
        trex_traffic_gen.options = {
        ...
        expected = [0.8, 1.0]
        result = trex_traffic_gen._get_rfc_tolerance()
    ...
    @mock.patch("yardstick.network_services.vnf_generic.vnf.tg_rfc2544_trex.time")
    @mock.patch(SSH_HELPER)
    def old_test__traffic_runner(self, ssh, *_):
        mock_ssh(ssh)
        ...
        mock_stl_client = mock.Mock(autospec=STLClient)
        mock_stl_client.get_stats.return_value = "0"

        sut = TrexTrafficGenRFC('vnf1', self.VNFD_0)
        sut.tg_port_pairs = [[[0], [1]]]
        sut.vnf_port_pairs = [[[0], [1]]]
        sut._vpci_ascending = ["0000:05:00.0", "0000:05:00.1"]
        sut.tc_file_name = get_file_abspath("tc_baremetal_rfc2544_ipv4_1flow_64B.yaml")
        sut.topology = "nsb_test.yaml"
        sut.options = {
            'packetsize': 64,
            'traffic_type': 4,
            'rfc2544': {
        ...
        sut.resource_helper._connect = mock.Mock(return_value=mock_stl_client)
        sut.resource_helper.rfc2544_helper.iteration.value = 0
        sut.resource_helper.rfc2544_helper._latency = False
    ...
    @mock.patch("yardstick.network_services.vnf_generic.vnf.tg_rfc2544_trex.time")
    @mock.patch(SSH_HELPER)
    def old_test__traffic_runner_latency(self, ssh):
        mock_ssh(ssh)
        ...
        mock_traffic_profile.execute.return_value = "64"
        mock_traffic_profile.get_drop_percentage.return_value = "64"
        mock_traffic_profile.calculate_pps.return_value = [1234, 0.0]
        ...
        sut.resource_helper._connect = mock.Mock(return_value=mock_stl_client)
        sut.resource_helper.rfc2544_helper.iteration.value = 11
        sut.resource_helper.rfc2544_helper._latency = True

        sut._traffic_runner(mock_traffic_profile)

    def old_test__split_mac_address_into_list(self):
        with mock.patch(SSH_HELPER) as ssh:
            ssh_mock = mock.Mock(autospec=ssh.SSH)
            ssh_mock.execute = mock.Mock(return_value=(0, "", ""))
            ssh.from_node.return_value = ssh_mock
            trex_traffic_gen = TrexTrafficGenRFC('vnf1', self.VNFD_0)
            expected = ['0x00', '0x00', '0x00', '0x00', '0x00', '0x01']
            result = trex_traffic_gen._split_mac_address_into_list("00:00:00:00:00:01")
            self.assertEqual(result, expected)
    ...
    def old_test__connect_client(self):
        with mock.patch(SSH_HELPER) as ssh:
            ssh_mock = mock.Mock(autospec=ssh.SSH)
            ssh_mock.execute = mock.Mock(return_value=(0, "", ""))
            ssh.from_node.return_value = ssh_mock
            ...
            self.assertIsNotNone(trex_traffic_gen._connect(client))
