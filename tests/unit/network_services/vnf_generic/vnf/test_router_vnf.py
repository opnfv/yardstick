...
class TestRouterVNF(unittest.TestCase):
    ...
    @mock.patch(SSH_HELPER)
    def test_run_router(self, ssh, mock_process):
        mock_ssh(ssh)

        vnfd = self.VNFD['vnfd:vnfd-catalog']['vnfd'][0]
        router_vnf = RouterVNF(name, vnfd)
        router_vnf._build_config = mock.MagicMock()
        router_vnf.queue_wrapper = mock.MagicMock()
        router_vnf.scenario_helper.scenario_cfg = self.scenario_cfg
        # router_vnf.vnf_cfg = {'lb_config': 'SW',
        #                           'lb_count': 1,
        #                           'worker_config': '1C/1T',
        #                           'worker_threads': 1}
        router_vnf.all_options = {'traffic_type': '4',
    ...
    def test_instantiate(self, ssh, mock_context, mock_process):
        ...
        vnfd = self.VNFD['vnfd:vnfd-catalog']['vnfd'][0]
        router_vnf = RouterVNF(name, vnfd)
        router_vnf.deploy_helper = mock.MagicMock()
        router_vnf.resource_helper = mock.MagicMock()
        router_vnf._build_config = mock.MagicMock()
    ...
    def test_terminate(self, ssh, mock_time, mock_process):
        ...
        router_vnf._resource_collect_stop = mock.Mock()
