...
if stl_patch:
    from yardstick.network_services.vnf_generic.vnf.prox import Prox, PacketDump


@mock.patch('yardstick.network_services.vnf_generic.vnf.prox.time')
class TestProx(unittest.TestCase):

    @mock.patch('yardstick.network_services.vnf_generic.vnf.prox.select')
    def test_get_data(self, mock_select, mock_time):
        mock_select.select.return_value = [1]
        mock_socket = mock.MagicMock()
        mock_socket.recv().decode.return_value = "\n"
        prox = Prox(mock_socket)
        ret = prox.get_data()
        self.assertEqual(ret, "")

    def test_put_data(self, mock_time):
        mock_socket = mock.MagicMock()
        prox = Prox(mock_socket)
        prox.put_data("data")
