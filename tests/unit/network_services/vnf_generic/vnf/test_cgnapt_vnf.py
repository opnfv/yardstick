...
class TestCgnaptApproxVnf(unittest.TestCase):
    ...
    def test_instantiate_panic(self, mock_process):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ...
            cgnapt_vnf.WAIT_TIME = 0
            cgnapt_approx_vnf.resource_helper = mock.MagicMock()
            cgnapt_approx_vnf._build_config = mock.MagicMock()
            cgnapt_approx_vnf._vnf_process = mock.MagicMock()
            cgnapt_approx_vnf._vnf_process.start = mock.Mock()
            cgnapt_approx_vnf._vnf_process.is_alive = mock.Mock(return_value=True)
            self.assertRaises(ValueError, cgnapt_approx_vnf.instantiate,
    ...
    @mock.patch("yardstick.network_services.vnf_generic.vnf.sample_vnf.time")
    @mock.patch("yardstick.network_services.vnf_generic.vnf.cnapt_vnf.time")
