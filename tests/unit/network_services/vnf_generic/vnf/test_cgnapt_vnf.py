...
class TestCgnaptApproxVnf(unittest.TestCase):
    ...
    def test__get_cgnapt_config(self):
        ...
        cgnapt_approx_vnf._get_ports_gateway = mock.Mock(return_value="4.4.5.4")
        self.assertEqual(["4.4.5.4"],
                         cgnapt_approx_vnf._get_cgnapt_config(interfaces))

    def test__get_random_public_pool_ip(self):
        vnfd = self.VNFD['vnfd:vnfd-catalog']['vnfd'][0]
        cgnapt_approx_vnf = CgnaptApproxVnf(vnfd)
        cgnapt_approx_vnf.execute_command = mock.Mock()
        interfaces = vnfd["vdu"][0]['external-interface']
        ...
        cgnapt_approx_vnf.vnf_cfg = {'lb_config': 'HW', 'worker_threads': 1}
        self.assertEqual("4.4.5.4",
                         cgnapt_approx_vnf._get_random_public_pool_ip("4.4.4.4"))
    ...
    def test_run_vcgnapt(self, MultiPortConfig, hex, eval, mock_open):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ssh_mock = mock.Mock(autospec=ssh.SSH)
            ssh_mock.execute = \
                mock.Mock(return_value=(0, "", ""))
            ...
            cgnapt_approx_vnf._append_routes = mock.Mock()
            cgnapt_approx_vnf._append_nd_routes = mock.Mock()
            cgnapt_approx_vnf._update_traffic_type = mock.Mock()
            cgnapt_approx_vnf._update_packet_type = mock.Mock()

            self.assertEqual(None,
                             cgnapt_approx_vnf._run_vcgnapt(queue_wrapper))

    @mock.patch('yardstick.network_services.vnf_generic.vnf.cgnapt_vnf.os')
    def test__provide_config_file(self, os):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ...
            ssh.from_node.return_value = ssh_mock
            cgnapt_approx_vnf = CgnaptApproxVnf(vnfd)
            cgnapt_approx_vnf._read_yang_model_config = mock.Mock(return_value=1)
            cgnapt_approx_vnf._get_converted_yang_entries = mock.Mock(return_value=1)
            prefix = 'acl_config'
            template = mock.MagicMock()
            cgnapt_approx_vnf.connection = ssh_mock
            self.assertIsNotNone(cgnapt_approx_vnf._provide_config_file(prefix,
                                                                    template))
    ...
    def test__validate_cpu_cfg(self, CpuSysCores):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ...
            self.scenario_cfg['vnf_options'] = {'acl': {'cfg': "",
                                                        'rules': ""}}
            cgnapt_approx_vnf._run_acl = mock.Mock(return_value=0)
            cgnapt_approx_vnf._parse_rule_file = mock.Mock(return_value={})
            ...
            cgnapt_approx_vnf.resource.stop = mock.Mock()
            cgnapt_approx_vnf.resource.amqp_process_for_nfvi_kpi = mock.Mock()
            cgnapt_approx_vnf.resource.check_if_sa_running(return_value=[1])
            cgnapt_approx_vnf.resource.amqp_collect_nfvi_kpi(return_value={})
            cgnapt_approx_vnf.generate_port_pairs = mock.Mock()
            cgnapt_approx_vnf.tg_port_pairs = [[[0], [1]]]
            cgnapt_approx_vnf.topology = ""
            vnf_cfg = {'lb_config': 'SW', 'worker_threads': 2}
            CpuSysCores.get_core_socket = mock.Mock(return_value=[{'0': ['1', '2']}])
            cgnapt_approx_vnf._get_cpu_sibling_list = mock.Mock(return_value=1)
            self.assertEqual(1, cgnapt_approx_vnf._validate_cpu_cfg(vnf_cfg))
    ...
    def test_instantiate_panic(self, Context):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ...
            cgnapt_approx_vnf.q_out.put("PANIC")
            cgnapt_vnf.WAIT_TIME = 3
            self.assertRaises(RuntimeError, cgnapt_approx_vnf.instantiate,
                              self.scenario_cfg, self.context_cfg)
    ...
    def test__get_ports_gateway(self):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ...
            self.assertEqual('152.16.100.20',
                             cgnapt_approx_vnf._get_ports_gateway('xe0'))
    ...
    def test__get_cpu_sibling_list(self):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ...
            self.assertEqual([], cgnapt_approx_vnf._get_cpu_sibling_list(cores))
            cores = ['1']
            cgnapt_approx_vnf.connection = ssh_mock
            self.assertEqual(['1', '2'], cgnapt_approx_vnf._get_cpu_sibling_list(cores))
