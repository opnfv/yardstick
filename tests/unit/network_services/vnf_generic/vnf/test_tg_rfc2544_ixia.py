...
class TestIXIATrafficGen(unittest.TestCase):
    ...
    def test___init__(self, mock_ixnextgen):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ...
            self.assertEqual(ixnet_traffic_gen._result, {})
    ...
    def test__get_rfc_tolerance(self, mock_ixnextgen):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ssh_mock = mock.Mock(autospec=ssh.SSH)
            ssh_mock.execute = \
                mock.Mock(return_value=(0, "", ""))
            ...
            self.assertEqual([0.8, 1.0],
                             ixnet_traffic_gen._get_rfc_tolerance())
    ...
    def test_run_traffic(self, mock_ixnextgen):
        ...
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ...
            vnfd["mgmt-interface"]["tg-config"].update({"py_bin_path":
                                                        "/root"})
            self.sut = IxiaTrafficGen(NAME, vnfd)
...
#    @mock.patch("yardstick.network_services.vnf_generic.vnf.tg_rfc2544_ixia.open")
    def test_traffic_runner(self, mock_ixnextgen):
        mock_traffic_profile = mock.Mock(autospec=TrafficProfile)
        mock_traffic_profile.get_traffic_definition.return_value = "64"
        mock_traffic_profile.params = self.TRAFFIC_PROFILE
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ...
            self.sut._ixia_traffic_gen = mock.MagicMock()
            self.sut._ixia_traffic_gen.ix_load_config = mock.MagicMock()
            self.sut._ixia_traffic_gen.ix_stop_traffic = mock.MagicMock()
            tg_rfc2544_ixia.WAIT_AFTER_CFG_LOAD = 1
            tg_rfc2544_ixia.WAIT_FOR_TRAFFIC = 1
            self.sut._ixia_traffic_gen.ix_get_statistics = \
