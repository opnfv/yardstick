...
class TestAclApproxVnf(unittest.TestCase):
    ...
    def test_collect_kpi(self):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ...
            acl_approx_vnf.resource.check_if_sa_running = \
                mock.Mock(return_value=[0, 1])
            acl_approx_vnf.resource.amqp_collect_nfvi_kpi= \
    ...
    def test_execute_command(self):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ...
            acl_approx_vnf = AclApproxVnf(vnfd)
            cmd = "quit"
            self.assertEqual("", acl_approx_vnf.execute_command(cmd))

    def test_get_stats_acl(self):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            vnfd = self.VNFD['vnfd:vnfd-catalog']['vnfd'][0]
            ssh_mock = mock.Mock(autospec=ssh.SSH)
            ssh_mock.execute = \
            ...
            acl_approx_vnf.execute_command = \
                mock.Mock(return_value=mock_result)
            self.assertEqual(mock_result, acl_approx_vnf.get_stats_acl())
    ...
    def test_run_acl(self, MultiPortConfig, hex, eval, mock_open):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ssh_mock = mock.Mock(autospec=ssh.SSH)
            ssh_mock.execute = \
                mock.Mock(return_value=(0, "", ""))
            ...
            acl_approx_vnf = AclApproxVnf(vnfd)
            curr_path = os.path.dirname(os.path.abspath(__file__))
            acl_approx_vnf.vnf_cfg = os.path.join(curr_path, "vpe_config")
            queue_wrapper = \
                QueueFileWrapper(acl_approx_vnf.q_in,
                                 acl_approx_vnf.q_out, "pipeline>")
            acl_approx_vnf.tc_file_name = \
            ...
            acl_approx_vnf._append_routes = mock.Mock()
            acl_approx_vnf._append_nd_routes = mock.Mock()
            acl_approx_vnf._update_traffic_type = mock.Mock()
            acl_approx_vnf._update_packet_type = mock.Mock()
    ...
    def test__parse_rule_file(self):
        ...
        acl_approx_vnf._read_yang_model_config = mock.Mock(return_value=1)
        acl_approx_vnf._get_converted_yang_entries = mock.Mock(return_value=1)
        self.assertEqual(1, acl_approx_vnf._parse_rule_file("/tmp/rule"))

    def test__parse_rule_file_fail(self):
        vnfd = self.VNFD['vnfd:vnfd-catalog']['vnfd'][0]
        acl_approx_vnf = AclApproxVnf(vnfd)
        acl_approx_vnf._read_yang_model_config = mock.Mock(return_value=0)
        self.assertEqual("", acl_approx_vnf._parse_rule_file("/tmp/rule"))

    def test__read_yang_model_config(self):
        vnfd = self.VNFD['vnfd:vnfd-catalog']['vnfd'][0]
        acl_approx_vnf = AclApproxVnf(vnfd)
        acl_approx_vnf._read_yang_model_config("tmp1_rule")

    def test__get_converted_yang_entries(self):
        vnfd = self.VNFD['vnfd:vnfd-catalog']['vnfd'][0]
        acl_approx_vnf = AclApproxVnf(vnfd)
        read_yaml = \
            acl_approx_vnf._read_yang_model_config(
                self._get_file_abspath("acl_1rule.yaml"))
        self.assertIsNotNone(
            acl_approx_vnf._get_converted_yang_entries(read_yaml))

    def test_deploy_acl_vnf(self):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            vnfd = self.VNFD['vnfd:vnfd-catalog']['vnfd'][0]
            ssh_mock = mock.Mock(autospec=ssh.SSH)
            ssh_mock.execute = \
            ...
            vnfd = self.VNFD['vnfd:vnfd-catalog']['vnfd'][0]
            acl_approx_vnf = AclApproxVnf(vnfd)
            acl_approx_vnf.deploy = mock.MagicMock()
            acl_approx_vnf.deploy.deploy_vnfs = mock.Mock()
            acl_approx_vnf.connection = ssh_mock
    ...
    def test__validate_cpu_cfg(self, CpuSysCores):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ...
            self.assertEqual(1, acl_approx_vnf._validate_cpu_cfg(vnf_cfg))
    ...
    def test_setup_vnf_environment(self):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ...
            acl_approx_vnf = AclApproxVnf(vnfd)
            self.assertEqual(None,
                             acl_approx_vnf.setup_vnf_environment(ssh_mock))
    ...
    def test_terminate(self):
        with mock.patch("yardstick.ssh.SSH") as ssh:
            ...
            acl_approx_vnf.used_drivers = {"01:01.0": "i40e",
                                           "01:01.1": "i40e"}
            acl_approx_vnf.execute_command = mock.Mock()
            acl_approx_vnf.connection = ssh_mock
