...
class TestProxApproxVnf(unittest.TestCase):
    ...
    def test_collect_kpi(self, ssh, mock_time):
        ...
        prox_approx_vnf = ProxApproxVnf(NAME, self.VNFD0)
        prox_approx_vnf.resource_helper.resource = mock.MagicMock()
        prox_approx_vnf.resource_helper.resource.check_if_sa_running = mock.Mock(return_value=[
                                                                                 0, 1])
        prox_approx_vnf.resource_helper.resource.amqp_collect_nfvi_kpi = mock.Mock(return_value={})
        result = {
            'packets_in': 0,
            'packets_out': 0,
            'collect_stats': {'core': {}},
        ...
        self.assertEqual(result, prox_approx_vnf.collect_kpi())
    ...
    def test_run_prox(self, ssh, mock_time):
        ...
        mgmt_interface = self.VNFD0['mgmt-interface']
        config_path = self.SCENARIO_CFG['vnf_options']["vnf__1"]["prox_config"]
        prox_path = self.SCENARIO_CFG['vnf_options']["vnf__1"]["prox_path"]
        prox_args = self.SCENARIO_CFG['vnf_options']["vnf__1"]["prox_args"]
        prox_approx_vnf.WAIT_TIME = 0
        prox_approx_vnf._run_prox(filewrapper, mgmt_interface, config_path,
                                  prox_path, prox_args)
        self.assertEqual(ssh.from_node.return_value.run.call_args[0][0],
    ...
    @mock.patch('yardstick.ssh.SSH')
    def test_instantiate(self, ssh, mock_find, mock_time):
        mock_ssh(ssh)

        prox_approx_vnf = ProxApproxVnf(NAME, self.VNFD0)
        prox_approx_vnf.connection = mock.MagicMock(
            **{"execute.return_value": (0, "", ""), "bin_path": ""})
        prox_approx_vnf._run_prox = mock.MagicMock(return_value=0)
        prox_approx_vnf.get_cpu_topology = mock.MagicMock(return_value={})
        prox_approx_vnf.find_used_drivers = mock.MagicMock(return_value={})
    ...
    def test_terminate(self, ssh, mock_socket, mock_time):
        ...
        prox_approx_vnf.rebind_drivers = mock.Mock()
        prox_approx_vnf.resource_helper = mock.MagicMock()
