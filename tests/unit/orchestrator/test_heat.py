...
class HeatTemplateTestCase(unittest.TestCase):
    ...
    def test_add_router(self):
        self.template.add_router('some-router', 'ext-net', 'some-subnet', provider='sriov')

        self.assertEqual(self.template.resources['some-router']['type'], 'OS::Neutron::Router')
        self.assertNotIn('depends_on', self.template.resources['some-router'])

    def test_add_router_interface(self):
        self.template.add_router_interface('some-router-if', 'some-router', 'some-subnet', provider='sriov')

        self.assertEqual(self.template.resources['some-router-if']['type'], 'OS::Neutron::RouterInterface')
        self.assertEqual(self.template.resources['some-router-if']['properties']['subnet_id'], 'some-subnet')
