...
#        self.heat_object._get_heat_client()

        self.heat_stack = heat.HeatStack("tmpStack")
        self.heat_stack.stacks_exist()
#        self.heat_stack.delete_all()

#        self.heat_tmp_stack2 = heat.HeatStack("tmpStack2")
#        self.heat_tmp_stack2.delete(block=False, retries=1)

#        self.heat_tmp_stack3 = heat.HeatStack("tmpStack3")
#        self.heat_tmp_stack3.delete(block=True, retries=1)

        self.file = open("/tmp/tmpfile","w")
        self.file.write("heat_template_version: 2015-04-30")
        self.file.close()
        self.heat_tmp_template = heat.HeatTemplate(self.heat_tmp_object,"/tmp/tmpfile",heat_parameters={"dict1": 1, "dict2": 2})


        self.heat_template = heat.HeatTemplate(self.heat_object)
        self.heat_template.resources = {}

#        self.heat_template.add_keypair("foo-key","1234")
