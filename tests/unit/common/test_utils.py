...
class TestUtils(unittest.TestCase):
    ...
    def test_get_free_port(self, mock_socket, mock_randint):
        ...
        with mock_socket('x', 'y') as s:
            s.side_effect = iter([0, 1])
    ...
    @mock.patch('subprocess.Process')
    def test_source_env(self, mock_process_type):
        base_env = deepcopy(os.environ)
        mock_process = mock_process_type()
        output_list = [
            'garbage line before',
            'NEW_ENV_VALUE=234',
            'garbage line after',
...
class TestUtilsIpAddrMethods(unittest.TestCase):
    ...
    INVALID_IP_ADDRESS_STR_LIST = [
        ...
        'w.x.y.z',
        '10.20.30.40/33',
        '123:4567:89ab:cdef:123:4567:89ab:cdef/129',
    ]
    ...
    def test_safe_ip_address_negative(self):
    ...
    def test_get_ip_version_negative(self):
        for value in self.INVALID_IP_ADDRESS_STR_LIST:
    ...
    def test_ip_to_hex_negative(self):
        addr_list = self.GOOD_IP_V4_ADDRESS_STR_LIST
        mask_list = self.GOOD_IP_V4_MASK_STR_LIST
