...
You can choose to run these sets of tests individually by choosing and
environment by passing the following arguments to ``tox``:
...
* ``-e pep8-full``: Linter and code checks for ALL files
...
Tox uses `virtualenv`_ to isolated Python environments to run the tests in.
The virtual environments are located at
...
This can also be achieved by removing the environments manually by deleting
the virtual environments before running ``tox``::
...
Writing tests
~~~~~~~~~~~~~
For each change submitted, a set of unit tests should be submitted, unit tests
should include both positive and negative testing.

In order to help identify which tests are needed, folllow the guidelines below.
...
The comment gives more information on the nature of the test, or the side
...
Mocking
+++++++
The ``mock`` library is used extensively for unit testing. It allows stubbing out of external libraries.
It is used extensively in Yardstick unit tests.
...
* Avoid decorating classes with mocks. Apply the mocking in ``setUp()`` instead to keep the scope smaller::
  ...
  should be::
    ...
    class MyClassTestCase(unittest.TestCase):
        def setUp(self):
            ...
            self.mock_ssh = self.mock_ssh.start()
