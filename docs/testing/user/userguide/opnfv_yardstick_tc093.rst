...
+-----------------------------------------------------------------------------+
|SDN Vswitch resilience in non-HA configuration                               |
|                                                                             |
+--------------+--------------------------------------------------------------+
...
|              | highly available in the event of Virtual Switch failure,     |
|              | in both compute nodes even if they are deployed in a non-HA  |
|              | or HA configuration. Specifically, the test verifies that    |
...
|              | between the existing VMs while the Virtual Switches are      |
|              | rebooting.                                                   |
|              |                                                              |
...
|              | that fails and starts again the VSwitch services which are   |
...
|              |     recovered                                                |
|              |                                                              |
|              |  2. outage_time: measures the total time where the           |
