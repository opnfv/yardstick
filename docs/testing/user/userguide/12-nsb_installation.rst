...
Network Service Benchmarking - Heat/SRIOV
-----------------------------------------

This section describes how to run vFW sample VNF TC in heat context
with SRIOV ports support enabled in the OpenStack.
...
.. warning:: The following configuration requires sudo access to the system. Make
  sure that your user have the access. Please contact the lab support in case
  you don't have it.

Enable The VT-d extension in the BIOS. Some system manufacturers disable these
extensions by default.

Activate Intel VT-d in the kernel by appending the ``intel_iommu=on`` parameter
to the kernel command line in the ``/etc/default/grub`` file. The snippet of the
``/etc/default/grub`` file with Intel VT-d activated are below:
...
.. code:: bash
  ...
  GRUB_CMDLINE_LINUX_DEFAULT="intel_iommu=on default_hugepagesz=2M hugepagesz=2M hugepages=3000"
  GRUB_CMDLINE_LINUX=""
  ...
...
Make sure that Intel VT-d has been enabled:
...
Setup SRIOV ports on the host:

.. note:: The ``enp24s0f0``, ``enp24s0f0`` are PF interfaces on a host and
  ``enp24s0f3`` is a public interface used in OpenStack, so the interface names
  should be changed according to the HW environment used for testing.
...
.. warning:: The rest of the configuration in the guide should be performed
  under stack user.
...
Create DevStack configuration file in ``devstack`` folder.

.. note:: Update the devstack configuration file. Lines that need to be
  updated are marked as a ``# CHANGE:`` comment.

.. include:: single-devstack-local.conf
   :code: console

Create external bridges for the physical networks:

.. note:: The ``enp24s0f0``, ``enp24s0f0`` are PF interfaces on a host, so
  the names should be changed according to the HW environment used for testing.

.. code:: bash

  sudo ovs-vsctl del-br br-enp24s0f0
  sudo ovs-vsctl del-br br-enp24s0f1
  sudo ovs-vsctl show
  sudo ovs-vsctl add-br br-enp24s0f0
  sudo ovs-vsctl add-port br-enp24s0f0 enp24s0f0
  sudo ovs-vsctl add-br br-enp24s0f1
  sudo ovs-vsctl add-port br-enp24s0f1 enp24s0f1
  sudo ovs-vsctl show

Start the devstack installation on the host by running the following
command :
...
.. note:: The ``ip``, ``user``, ``password`` and ``vpci`` fields show be  changed
  accoding to HW environment used for the testing. Use ``lshw -c network -businfo``
  command to get the PF PCI address for ``vpci`` field.

.. include:: single-yardstick-pod.conf
   :code: console

Run sample vFW RFC2544 SRIOV TC (``samples/vnf_samples/nsut/vfw/
...
`Add stack user`_ and `download devstack`_ on the controller and compute
hosts.

Create DevStack configuration file in ``devstack`` folder on the controller host:

.. note:: Update the devstack configuration file. Lines that need to be
  updated are marked as a ``# CHANGE:`` comment.

.. include:: multi-devstack-controller-local.conf
   :code: console

Create DevStack configuration file in ``devstack`` folder on the compute host:

.. note:: Update the devstack configuration file. Lines that need to be
  updated are marked as a ``# CHANGE:`` comment.

.. include:: multi-devstack-compute-local.conf
   :code: console

Start the devstack installation on the controller and compute hosts by running
