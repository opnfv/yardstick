...
|              |                                                              |
|              | The workflow for the test is to first create a Heat stack    |
|              | with the specified number of VMs and attaches Cinder         |
|              | volumes to each VM.  Once the stack is created, each         |
|              | volume is filled with random data to ensure reads and        |
|              | subsequent writes show performance indicative of continued   |
|              | usage, and not skewed numbers while the disk fills with      |
|              | data.  The metrics for filling the volumes with random data  |
