...
.. table:: Description of the parameters used with ``docker`` command
...
in the following sections. Before that, access to the Yardstick container::
...
.. note:: Since Euphrates release, the above command will not able to
...
Manually create Yardstick flavor and guest images
#################################################
...
latency. You just need to type in
**yardstick testcase show opnfv_yardstick_tc002**, and the console would show
the config yaml of this test case::
...
If you want run a test case, then you need to use
**yardstick task start <test_case_path>**
...
To enable the access from a container to Internet using ``docker``, depends on
the OS version. On Ubuntu 14.04 LTS, which uses SysVinit,
``/etc/default/docker`` must be modified::
...
stop and delete the container (**be careful, everything on this container will
be lost**)::

   sudo docker stop yardstick
   sudo docker rm yardstick

Then follow the previous instructions to rebuild the Yardstick container.


References
----------

.. _`User Guide & Configuration Guide`: http://docs.opnfv.org/en/stable-danube/release/userguide.introduction.html#opnfv-user-config
