        ...
        for file_batch in file_batches:
            results.extend(
                cls.get_command_outputs(
                    [WrappedPopen(["flake8", "--ignore=E501", f], stdout=PIPE, stderr=PIPE)
                     for f in file_batch])
            )
...
class TestYardstick(Tester):
    ...
    # def run_pytest(self):
        # # run for test_ui_helpers.py, etc.
        # new_env = os.environ.copy()
        # # Add for Ixia tests
        # new_env.update(IXIA_ENV)
        # py_test_error = WrappedPopen(
        #     ['py.test', "-n", str(NUM_CPUS), "--junitxml=pytest.xml",
        #      # so we can validate the json
        #      '--configdir={}'.format('config'),
        #      self.UNITTEST_PREFIX],
        #     env=new_env).wait()
        # return self.process_errors([py_test_error])

TESTERS = {
    'csig_sdnd_benchmarking-industrial_standard_benchmarking': TestYardstick,
}
