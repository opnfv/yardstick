...
def create_neutron_net(shade_client, network_name, shared=False,
                       admin_state=True, external=False, provider=None,
    ...
    :param admin_state:(bool) set the network administrative state.
    ...
    try:
        ...
        log.exception("Error [create_neutron_net(shade_client)]")
        return None
...
def create_neutron_subnet(shade_client, network_name_or_id, cidr=None,
    ...
    try:
        ...
        log.exception("Error [create_neutron_subnet(shade_client)]")
        return None
...
def create_neutron_router(shade_client, name=None, admin_state_up=True,
    ...
    try:
        ...
        log.exception("Error [create_neutron_router(shade_client)]")
        return None
...
def remove_router_interface(shade_client, router, subnet_id=None,
    ...
    try:
        ...
        log.exception("Error [remove_interface_router(shade_client)]")
        return False
...
def create_floating_ip(shade_client, network_name_or_id=None, server=None,
    ...
    that the fixed IP to attach the floating IP to should be on.
    :param port:(optional) The port ID that the floating IP should be
    attached to. Specifying a port conflicts with specifying a server,
    fixed_address or nat_destination.
    ...
    try:
        ...
        log.exception("Error [create_floating_ip(shade_client)]")
        return None
