...
def create_instance_and_wait_for_active(shade_client, name, image=None,
                                        flavor=None, auto_ip=True, ips=None,
    ...
    :param volumes: A list of volumes to attach to the server.
    :param wait: Wait for the address to appear as assigned to the server.
    ...
    :param network_id:(string) Network id for multiple tenant network.
    :returns: the created server.
    """
    try:
        instance = shade_client.create_server(
            name=name, image=image, flavor=flavor, auto_ip=auto_ip,
            ips=ips, ip_pool=ip_pool, root_volume=root_volume,
            terminate_volume=terminate_volume, wait=wait, timeout=timeout,
            reuse_ips=reuse_ips, network=network,
            boot_from_volume=boot_from_volume, volume_size=volume_size,
            boot_volume=boot_volume, volumes=volumes,
            nat_destination=nat_destination, network_id=network_id)
        return instance
