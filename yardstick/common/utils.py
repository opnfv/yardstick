...
def get_os_version(ssh_client):
    ...
    :return tuple of str: Linux OS and kernel versions
    """
    _, os_ver, _ = ssh_client.execute("cat /etc/lsb-release")
    _, kernel_ver, _ = ssh_client.execute("uname -a")
    return os_ver, kernel_ver


def get_sample_vnf_info(ssh_client, json_file='/opt/nsb_bin/yardstick_sample_vnf.json'):
    ...
    for vnf_name, vnf_data in json_data.items():
        _, out, _ = ssh_client.execute("md5sum %s" % vnf_data["path_vnf"])
