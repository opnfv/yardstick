...
def _pip_main(package, action):
    if action == ACTION_UNINSTALL:
        return pip.main([action, package, '-y'])
    return pip.main([action, package])
...
def _pip_execute_action(package, action=ACTION_INSTALL):
    ...
    """
    try:
        status = _pip_main(package, action)
...
def pip_install(package):
    ...
    return _pip_execute_action(package, action=ACTION_INSTALL)
