...
if not PYTHONPATH:
    print("PYTHONPATH not setup. VNF testing aborted...")
    print("Please run 'source ~/.bash_profile' on shell to seutp PYTHONPATH")
    raise SystemExit(1)

if not VIRTUAL_ENV:
    print("Virtual environment not found.")
    print("Please run 'source /opt/nsb_bin/yardstick_venv/bin/activate'")
    raise SystemExit(1)
...
class YardstickNSCli(object):
    ...
    def run_test(self, args, test_path):
        ...
        try:
            ...
            command = 'yardstick --debug task start {0}'.format(test)
            output = subprocess.check_output(["yardstick", "--debug", "task", "start", test])
            ...
            print("Invalid input: %s." % error.message)
