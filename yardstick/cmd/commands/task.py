...
class TaskCommands(object):
    ...
    @cliargs("--suite", help="process test suite file instead of a task file",
             action="store_true")
    @cliargs("--test-cases-dir", type=str, help="directory where task files"
             "for the test suite are located, default %s"
             % test_cases_dir_default, default=test_cases_dir_default)
    def do_start(self, args):
        ...
        if args.suite:
            task_fnames, task_args, task_args_fnames = parser.parse_suite()
            if args.test_cases_dir[-1] != os.sep:
                args.test_cases_dir += os.sep
            task_files = [args.test_cases_dir + task for task in task_fnames]
        else:
...
class TaskParser(object):
    ...
    def parse_suite(self):
        ...
        return task_fnames, task_args, task_args_fnames
