    ...
    def parse_suite(self):
        ...
        for task in cfg["test_cases"]:
            ...
            if "constraint" in task:
                ...
                valid_task_files.append(test_cases_dir + task_fname)
            # 2.check task_args and args_file
            if task_args is not None:
                valid_task_args.append(task_args.get(cur_pod, None))
            else:
                valid_task_args.append(None)
            if task_args_fnames is not None:
                valid_task_args_fnames.append(
                    task_args_fnames.get(cur_pod, None))
            else:
                valid_task_args_fnames.append(None)

        LOG.info("******valid_task_files:%s, valid_task_args:%s," +
                 "valid_task_args_fnames:%s",
                 valid_task_files, valid_task_args, valid_task_args_fnames)
