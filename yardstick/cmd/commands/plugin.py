...
# plugin_install_script_dir_default = "yardstick/resources/script/install"
# plugin_remove_script_dir_default = "yardstick/resources/script/remove"
...
class PluginCommands(object):
    ...
    @cliargs("inputfile", type=str, help="path to plugin configuration file",
             nargs=1)
    def do_install(self, args):
        '''Install a plugin.'''

        total_start_time = time.time()
        parser = PluginParser(args.inputfile[0])

        # plugin_config_file = parser.path
        # plugin_config_name = os.path.splitext(os.path.basename
        #                                       (plugin_config_file))[0]
...
class PluginParser(object):
    '''Parser for plug config files in yaml format'''
