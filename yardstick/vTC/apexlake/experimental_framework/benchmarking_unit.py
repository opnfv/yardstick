...
class BenchmarkingUnit:
    ...
    def get_available_test_cases():
        """
        Returns a list of available test cases for the framework
        ...
        dir = '{}experimental_framework/benchmarks'.\
            format(common.get_base_dir())
        ...
        for dirname, dirnames, filenames in os.walk(dir):
            for file in filenames:
                if file.endswith('py'):
                    ...
                    modules = __import__(class_name, '', '', '', -1)
        benchmarks = getattr(modules, 'benchmarks')
        for mod in files:
            classes = inspect.getmembers(getattr(benchmarks, mod))
            for m in classes:
                if inspect.isclass(m[1]):
                    class_name = m[1]("", dict()).__class__.__name__
                    if isinstance(m[1]("", dict()), base.BenchmarkBaseClass) and \
