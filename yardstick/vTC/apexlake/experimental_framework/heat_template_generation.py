...
'''
This file contains the code to Generate the heat templates from the base template
'''
...
class TreeNode:
    ...
    Each node represents a single configuration value for a single configuration parameter.
    """
...
def generates_templates(base_heat_template, deployment_configuration):
    ...
    # Delete the templates eventually generated in previous running of the framework
    ...
    for leaf in leaves:
        ...
        common.LOG.debug("Heat Templates and Metadata file " + str(counter) + " created")
        counter += 1

    # Creation of the template files
    common.LOG.info(str(counter - 1) + " Heat Templates and Metadata files created")
...
def get_all_heat_templates(template_dir, template_file_extension):
    ...
    :param template_file_extension: extension of the file for templates (type: str)
    :return: type: list
    """
    template_files = list()
    for dirname, dirnames, filenames in os.walk(template_dir):
        for filename in filenames:
            if template_file_extension in filename and filename.endswith(template_file_extension) and \
