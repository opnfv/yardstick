...
class TestDpdkPacketGenSendTraffic(unittest.TestCase):
    ...
    @mock.patch('experimental_framework.packet_generators.'
                'dpdk_packet_generator.DpdkPacketGenerator._get_core_nics')
    @mock.patch('experimental_framework.packet_generators.'
                'dpdk_packet_generator.DpdkPacketGenerator._init_physical_nics')
    @mock.patch('experimental_framework.packet_generators.'
                'dpdk_packet_generator.DpdkPacketGenerator._finalize_physical_nics')
    @mock.patch('experimental_framework.packet_generators.'
    ...
    def test_send_traffic_for_success(self, mock_ch_dir,
        ...
        expected_command = 'pktgen_dir/program -c coremask -n memchannel ' \
                           '--proc-type auto --file-prefix pg -- ' \
                           '-T -P -m "{corenics}" ' \
                           '-f base_dir/experimental_framework/' \
                           'packet_generators/dpdk_pktgen/lua_file ' \
                           '-s 0:base_dir/experimental_framework/' \
                           'packet_generators/pcap_files/pcap_file ' \
                           '-s 1:base_dir/experimental_framework/' \
                           'packet_generators/pcap_files/pcap_file > /dev/null'
...
class TestDpdkPacketGenOthers(unittest.TestCase):
    ...
    def test__init_input_validation_for_success(self, mock_is_file):
        ...
        self.assertEqual(mut.DpdkPacketGenerator.
