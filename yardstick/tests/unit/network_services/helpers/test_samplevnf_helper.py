...
class TestMultiPortConfig(unittest.TestCase):
    ...
    def setUp(self):
        self._mock_open = mock.patch.object(builtins, 'open')
        self.mock_open = self._mock_open.start()
        self._mock_os = mock.patch('os.path')
        self.mock_os = self._mock_os.start()
        self._mock_config_parser = mock.patch(
            'yardstick.network_services.helpers.samplevnf_helper.ConfigParser')
    ...
    def test_validate_ip_and_prefixlen(self):
        ip_addr, prefix_len = MultiPortConfig.validate_ip_and_prefixlen(
            '10.20.30.40', '16')
