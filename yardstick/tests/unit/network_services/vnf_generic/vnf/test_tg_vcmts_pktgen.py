...
from yardstick.network_services.vnf_generic.vnf.tg_vcmts_pktgen import PktgenHelper
...
class TestPktgenHelper(unittest.TestCase):
    ...
    def test___init__(self):
        pktgen_helper = PktgenHelper("localhost", 23000)
...
class TestVcmtsPktgen(unittest.TestCase):
    ...
    def test_extract_pod_cfg(self):
        vnfd = self.VNFD['vnfd:vnfd-catalog']['vnfd'][0]
        vcmts_pktgen = tg_vcmts_pktgen.VcmtsPktgen(NAME, vnfd, 'task_id')
        pod_cfg = vcmts_pktgen.extract_pod_cfg(self.PKTGEN_POD_VALUES, "0")
        self.assertIsNotNone(pod_cfg)
        self.assertEqual(pod_cfg["pktgen_id"], "0")
        pod_cfg = vcmts_pktgen.extract_pod_cfg(self.PKTGEN_POD_VALUES, "4")
    ...
    def test_run_traffic(self):
        ...
        vcmts_pktgen.pktgen_helper.connect.return_value = False
        with self.assertRaises(IOError):
            vcmts_pktgen.run_traffic({})
