...
class TestVcmtsPktgenSetupEnvHelper(unittest.TestCase):
    ...
    def test_generate_pcap_filename(self):
        pcap_file_name = self.setup_helper.generate_pcap_filename(\
                            TestVcmtsPktgen.PKTGEN_VALUES[0]['ports'][0])
...
class TestVcmtsPktgen(unittest.TestCase):
    ...
    VNFD = {'vnfd:vnfd-catalog':
            ...
            "mgmt-interface": {
                "ip": "192.168.24.150",
                "key_filename": "/home/opnfv/repos/yardstick/yardstick/resources/files/yardstick_key-a3b663c2",
                "user": "root",
    ...
    def test___init__(self):
        ...
        vcmts_pktgen = tg_vcmts_pktgen.VcmtsPktgen(NAME, vnfd, 'task_id')
        self.assertFalse(vcmts_pktgen.traffic_finished)
        self.assertIsNotNone(vcmts_pktgen.setup_helper)
        self.assertIsNotNone(vcmts_pktgen.resource_helper)

    def test_extract_pod_cfg(self):
        vnfd = self.VNFD['vnfd:vnfd-catalog']['vnfd'][0]
        vcmts_pktgen = tg_vcmts_pktgen.VcmtsPktgen(NAME, vnfd, 'task_id')
        pod_cfg = vcmts_pktgen.extract_pod_cfg(self.PKTGEN_VALUES, "0")
        self.assertIsNotNone(pod_cfg)
        self.assertEquals(pod_cfg["pktgen_id"], "0")
        pod_cfg = vcmts_pktgen.extract_pod_cfg(self.PKTGEN_VALUES, "4")
    ...
    def test_instantiate(self, *args):
        ...
        vcmts_pktgen._start_server = mock.Mock(return_value=0)
        vcmts_pktgen.resource_helper = mock.MagicMock()
        vcmts_pktgen.setup_helper = mock.MagicMock()

        vcmts_pktgen.instantiate(self.SCENARIO_CFG, self.CONTEXT_CFG)
        self.assertIsNotNone(vcmts_pktgen.pod_cfg)
        self.assertEqual(vcmts_pktgen.pod_cfg["pktgen_id"], "0")

        err_scenario_cfg = self.SCENARIO_CFG
        err_scenario_cfg['options'].pop('pktgen_values', None)
        with self.assertRaises(KeyError):
            vcmts_pktgen.instantiate(err_scenario_cfg, self.CONTEXT_CFG)
    ...
    def test_run_traffic(self):
        vnfd = self.VNFD['vnfd:vnfd-catalog']['vnfd'][0]
        vcmts_pktgen = tg_vcmts_pktgen.VcmtsPktgen(NAME, vnfd, 'task_id')
        vcmts_pktgen.pktgen_helper = mock.MagicMock()
        vcmts_pktgen.pktgen_helper.connect.return_value = True
        self.assertTrue(vcmts_pktgen.run_traffic({}))
        vcmts_pktgen.pktgen_helper.connect.assert_called_once()
        vcmts_pktgen.pktgen_helper.send_command.assert_called_with(
            'pktgen.start("all");')
