...
class TestVppSetupEnvHelper(unittest.TestCase):
    ...
    def test_get_vpp_interface_by_mac_l2_error(self):
        ...
        self.assertIn(
            'key l2_address not found in interface dict.Probably input list is not parsed from correct VAT json output.',
            str(raised.exception))
    ...
    def test_get_vpp_interface_by_mac_l2_length_error(self):
        ...
        self.assertIn(
            'key l2_address_length not found in interface dict. Probably input list is not parsed from correct VAT json output.',
            str(raised.exception))
...
class TestVatTerminal(unittest.TestCase):
    ...
    def test___init___error(self):
        def interactive_terminal_open():
            raise Exception

        ssh_helper = mock.Mock()
        ssh_helper.interactive_terminal_open = interactive_terminal_open
    ...
    def test___init___exec_error(self):
        def interactive_terminal_exec_command():
            raise Exception

        ssh_helper = mock.Mock()
        ssh_helper.interactive_terminal_exec_command.return_value = str(
            {'empty': 'value'}).replace("\'", "\"")
        ssh_helper.interactive_terminal_exec_command = interactive_terminal_exec_command
        VatTerminal(ssh_helper, json_param=True)
    ...
    def test_vat_terminal_exec_cmd_error(self):
        ...
        with self.assertRaises(RuntimeError) as raised:
            vat_terminal.vat_terminal_exec_cmd(
                "hw_interface_set_mtu sw_if_index 1 mtu 9200")
        self.assertIn(
            'VPP not running on node. VAT command hw_interface_set_mtu sw_if_index 1 mtu 9200 execution failed',
            str(raised.exception))
    ...
    def test_vat_terminal_close_vat_error(self):
        ...
        ssh_helper = mock.Mock()
        ssh_helper.interactive_terminal_close = interactive_terminal_close
        vat_terminal = VatTerminal(ssh_helper, json_param=False)
        with self.assertRaises(RuntimeError) as raised:
            vat_terminal.vat_terminal_close()
