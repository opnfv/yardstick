...
class TestIXIATrafficGen(unittest.TestCase):
    ...
    @mock.patch("yardstick.network_services.vnf_generic.vnf.tg_rfc2544_ixia.Process")
    def test__init_traffic_process(self, mock_process, *args):
        vnfd = self.VNFD['vnfd:vnfd-catalog']['vnfd'][0]
        sut = tg_rfc2544_ixia.IxiaTrafficGen('vnf1', vnfd, 'task_id')
        sut.resource_helper = mock.Mock()
        sut.resource_helper.client_started.value = 1
        sut._init_traffic_process(self.TRAFFIC_PROFILE)
        sut._traffic_process.start.assert_called_once()
