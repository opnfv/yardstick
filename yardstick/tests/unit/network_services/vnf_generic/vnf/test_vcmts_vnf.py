...
class TestVcmtsVNF(unittest.TestCase):
    ...
    def test___init__(self, *args):
        ...
        vnf = vcmts_vnf.VcmtsVNF(NAME, vnfd, 'task_id')
        self.assertIsNotNone(vnf.setup_helper)

    def test_extract_pod_cfg(self):
        vnfd = self.VNFD['vnfd:vnfd-catalog']['vnfd'][0]
        vnf = vcmts_vnf.VcmtsVNF(NAME, vnfd, 'task_id')
        pod_cfg = vnf.extract_pod_cfg(self.POD_CFG, "0")
        self.assertIsNotNone(pod_cfg)
        self.assertEqual(pod_cfg['sg_id'], '0')
        pod_cfg = vnf.extract_pod_cfg(self.POD_CFG, "1")
    ...
    def test_instantiate(self, ssh, *args):
        ...
        with self.assertRaises(KeyError):
            vnf.instantiate(err_scenario_cfg, self.CONTEXT_CFG)
    ...
    def test__update_collectd_options(self):
        ...
        vnfd = self.VNFD['vnfd:vnfd-catalog']['vnfd'][0]
        vnf = vcmts_vnf.VcmtsVNF('vnf__0', vnfd, 'task_id')
        vnf._update_collectd_options(scenario_cfg, context_cfg)
        self.assertEqual(vnf.setup_helper.collectd_options, expected)
    ...
    def test__update_options(self):
        ...
        vnfd = self.VNFD['vnfd:vnfd-catalog']['vnfd'][0]
        vnf = vcmts_vnf.VcmtsVNF('vnf1', vnfd, 'task_id')
    ...
    def test_wait_for_instantiate(self):
        vnfd = self.VNFD['vnfd:vnfd-catalog']['vnfd'][0]
        vnf = vcmts_vnf.VcmtsVNF(NAME, vnfd, 'task_id')
        self.assertIsNone(vnf.wait_for_instantiate())

    def test_terminate(self):
        vnfd = self.VNFD['vnfd:vnfd-catalog']['vnfd'][0]
        vnf = vcmts_vnf.VcmtsVNF(NAME, vnfd, 'task_id')
        self.assertIsNone(vnf.terminate())

    def test_scale(self):
        vnfd = self.VNFD['vnfd:vnfd-catalog']['vnfd'][0]
        vnf = vcmts_vnf.VcmtsVNF(NAME, vnfd, 'task_id')
        self.assertIsNone(vnf.scale())

    def test_collect_kpi(self):
        vnfd = self.VNFD['vnfd:vnfd-catalog']['vnfd'][0]
        vnf = vcmts_vnf.VcmtsVNF(NAME, vnfd, 'task_id')

        vnf.influxdb_helper = mock.MagicMock()
        vnf.collect_kpi()
        vnf.influxdb_helper.copy_kpi.assert_called_once()

    def test_start_collect(self):
        vnfd = self.VNFD['vnfd:vnfd-catalog']['vnfd'][0]
        vnf = vcmts_vnf.VcmtsVNF(NAME, vnfd, 'task_id')

        vnf.vcmts_influxdb_ip = "localhost"
        vnf.vcmts_influxdb_port = 8800

        self.assertTrue(vnf.start_collect())
        self.assertIsNotNone(vnf.influxdb_helper)

    def test_stop_collect(self):
        vnfd = self.VNFD['vnfd:vnfd-catalog']['vnfd'][0]
        vnf = vcmts_vnf.VcmtsVNF(NAME, vnfd, 'task_id')
        self.assertIsNone(vnf.stop_collect())
