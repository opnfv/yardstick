...
class TestProxSocketHelper(unittest.TestCase):
    ...
    def test_multi_port_stats_diff(self, *args):
        ...
        prox.get_data = mock.MagicMock(return_value='0,2,4,6,8,10;1,4,8,16,32,64')
        result2 = prox.multi_port_stats([0, 1])

        expected = [[0, 0.0, 0.0, 0, 0, 5], [ 1, 0.0, 0.0, 0, 0, 59]]
        result , result3 = prox.multi_port_stats_diff(result1, result2, 1)

        self.assertEqual(result, expected)
        self.assertEqual(result3, result2)

        expected = [[0, 0.0, 0.0, 0, 0, 5], [1, 0.0, 0.0, 0, 0, 5]]
