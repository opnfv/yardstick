...
class SriovContextTestCase(unittest.TestCase):
    ...
    def test_setup_sriov_context(self, mock_create_vm, mock_write_file, mock_check,
                                 mock_build_vm_xml, mock_gen_cdrom_image, mock_add_cdrom, mock_check_update_key):
        self.sriov.servers = {
            'vnf_0': {
                'network_ports': {
                    'mgmt': {'cidr': '152.16.100.10/24'},
        ...
        mock_check_update_key.assert_called_once_with(connection, 'node_1', 'out_xml', key_filename)
