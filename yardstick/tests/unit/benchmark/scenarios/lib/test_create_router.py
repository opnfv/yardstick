...
class CreateRouterTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.result = {"router_create": 0}

        self._crouter_obj = create_router.CreateRouter(self.args, mock.ANY)
    ...
    def test_run(self):
        ...
        self._crouter_obj.scenario_cfg = {'output': 'id'}
        self.mock_create_neutron_router.return_value = _uuid
        output = self._crouter_obj.run(self.result)
        self.assertDictEqual({"router_create": 1}, self.result)
    ...
    def test_run_fail(self):
        ...
        with self.assertRaises(exceptions.ScenarioCreateRouterError):
            self._crouter_obj.run(self.result)
        self.assertDictEqual({"router_create": 0}, self.result)
