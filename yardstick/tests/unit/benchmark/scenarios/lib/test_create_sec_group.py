...
class CreateSecurityGroupTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._csecgp_obj = create_sec_group.CreateSecgroup(self.args, mock.ANY)
    ...
    def test_run(self):
        ...
        self._csecgp_obj.scenario_cfg = {'output': 'id'}
        self.mock_create_security_group_full.return_value = _uuid
        output = self._csecgp_obj.run(self.result)
    ...
    def test_run_fail(self):
        ...
        with self.assertRaises(exceptions.ScenarioCreateSecurityGroupError):
            self._csecgp_obj.run(self.result)
