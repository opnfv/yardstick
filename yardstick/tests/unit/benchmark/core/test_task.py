...
class TaskTestCase(unittest.TestCase):
    ...
    def test_teardown_error_handler(self, mock_scenario, mock_import_module):
        t = task.Task()
        scenario_cfg = {
            "host": "athena.demo",
            "type": "Server",
        ...
        task.Task.teardown_error_handler(t, scenario_cfg)

        mock_scenario.get.assert_called_once()
        mock_import_module.assert_called_once()
        run_mock.run.assert_called_once()

        self.assertEqual(run_mock.run.return_value, t.outputs)
