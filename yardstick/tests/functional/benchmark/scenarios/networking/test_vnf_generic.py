...
class VnfGenericTestCase(unittest.TestCase):
    ...
    def test__render_topology_with_args(self):
        ...
        self.ns_testcase.scenario_cfg['extra_args'] = {'vports': 5}

        self.ns_testcase._render_topology()
        topology = self.ns_testcase.topology
        self.assertEqual("3tg-topology", topology['id'])
        vld = self.ns_testcase.topology[u'vld']
        self.assertEqual(len(vld), 6)

        self.assertEqual(u'uplink_0', vld[0][u'id'])
        self.assertEqual(u'tg__0 to vnf__0 link 1', vld[0][u'name'])
        self.assertEqual(u'xe0', vld[0][u'vnfd-connection-point-ref'][0][u'vnfd-connection-point-ref'])

        self.assertEqual(u'downlink_0', vld[1][u'id'])
        self.assertEqual(u'vnf__0 to tg__0 link 2', vld[1][u'name'])
        self.assertEqual(u'xe1', vld[1][u'vnfd-connection-point-ref'][0][u'vnfd-connection-point-ref'])
