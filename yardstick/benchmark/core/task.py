...
class Task(object):     # pragma: no cover
    ...
    def _parse_tasks(self, parser, task_files, args, task_args,
        ...
        if args.render_only:
            if not os.path.exists(args.render_only):
                os.makedirs(args.render_only)
            for idx, task in enumerate(tasks):
                output_file_name = os.path.abspath(os.path.join(
                    args.render_only,
                    '{0:03d}-{1}.txt'.format(idx, task['case_name'])))
    ...
    def _render_task(self, task_args, task_args_file):
        ...
        try:
            with open(self.path) as f:
                input_task = f.read()
                rendered_task = task_template.TaskTemplate.render(input_task,
                                                                  **kw)
