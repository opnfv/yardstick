...
class OvsDpdkContext(StandaloneBase):
    ...
    def _specific_cpu_handling(self):
        ...
        # if the pmd_thread_pin is specified, we want to pin the threads without allocating the
        # cores.
        # if the pmd_threads (i.e. just a number), we want to reserve the cores for the pmd
        # threads.
        pmd_threads = None
        # since the pmd_threads parameter was earlier in the ovs_properties, let's take it from
        # there as a fallback
