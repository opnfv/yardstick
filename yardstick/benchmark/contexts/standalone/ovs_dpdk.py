...
class OvsDpdkContext(Context):
    ...
    def setup_ovs(self):
        ...
        XARGS_KILL_TEMPLATE = "ps -ef | grep %s | grep -v grep | awk '{print $2}' | xargs -r kill %s"
    ...
    def setup_ovs_bridge_add_flows(self):
        ...
        for index, key in enumerate(ordered_network):
            vnf = ordered_network[key]
            if ovs_ver >= [2, 7, 0]:
    ...
    def check_ovs_dpdk_env(self):
        ...
        ovs_ver = version.get("ovs", "2.6.0")
        dpdk_ver = version.get("dpdk", "16.07.2")

        supported_version = self.SUPPORTED_OVS_TO_DPDK_MAP.get(ovs_ver, None)
        if supported_version is None or supported_version != dpdk_ver:
    ...
    def deploy(self):
        ...
        self.networks = self.helper.get_nic_details(self.connection,
                                                    self.networks,
                                                    self.dpdk_nic_bind)
    ...
    def setup_ovs_dpdk_context(self):
        ...
        for index, key in enumerate(OrderedDict(self.servers)):
            ...
            vcpu, mac = self.libvirt.build_vm_xml(self.connection,
                                                  self.vm_flavor, cfg,
                                                  vm_name, index)
            # 2: Cleanup already available VMs
            vnf = self.servers[key]
            for idx, vkey in enumerate(OrderedDict(vnf["network_ports"])):
                vfs = vnf["network_ports"][vkey]
                if vkey == "mgmt":
                    continue
