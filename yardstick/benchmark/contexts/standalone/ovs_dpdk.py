...
class OvsDpdkContext(Context):
    ...
    def setup_ovs_bridge_add_flows(self):
        ...
        dpdk_list = []
        vpath = self.ovs_properties.get("vpath", "/usr/local")
        ovs_ver = int(self.ovs_properties.get("version", {}).get("ovs", "2.6.0").replace(".", ""))
        ovs_add_port = \
        ...
        for index, key in enumerate(orderd_network):
            ...
            self.connection.execute(ovs_flow % (in_port, output))
            self.connection.execute(ovs_flow % (output, in_port))
