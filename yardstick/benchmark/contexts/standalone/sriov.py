...
def make_random_mac_addr(*octets):
    octets = itertools.chain(octets, itertools.repeat(iter(make_random_octet, None), 6))
...
class SriovStandaloneContext(StandaloneContext):
    ...
    def _install_required_libraries(self):
        ...
        for cmd in cmd_list:
            self._ssh_helper.execute(cmd, log.debug)

    def _configure_nics_for_sriov(self):
        node0, nic_details = self.get_nic_details()
        phy_driver = node0['phy_driver']
        self._ssh_helper.execute("rmmod {0}".format(phy_driver))
        self._ssh_helper.execute("modprobe {0} num_vfs=1".format(phy_driver))

        nic_details['vf_pci'] = {}
        keys = 'pci', 'interface', 'vf_mac'
        for index, (pci, interface, vf_mac) in enumerate(make_dict_iter(nic_details, keys)):
            cmd = "echo 1 > /sys/bus/pci/devices/{0}/sriov_numvfs"
            self._ssh_helper.execute(cmd.format(pci))

            cmd = "ip link set {interface} vf 0 mac {mac}"
            err, out, _ = self._ssh_helper.execute(cmd.format(interface, vf_mac))
    ...
    def _setup_xml_template(self):
        ...
        super(SriovStandaloneContext, self)._setup_xml_tempalte()
    ...
    def _blacklist_driver(self):
        ...
        if kwargs['vf_nic'] not in lines:
            ...
            self._ssh_helper.execute(cmd.format(**kwargs))

    def _enable_interfaces(self):
        node0, nic_details = self.get_nic_details()
        keys = 'interface', 'vf_pci'
        nic_iter = ((intf, vf['vf_pci']) for intf, vf in make_dict_iter(nic_details, keys))
        for index, (interface, vf_pci) in enumerate(nic_iter):
            self.add_sriov_interface(index, vf_pci, self.vm_mac_address, "/tmp/vm_sriov.xml")
            self._ssh_helper.execute("ifconfig {interface} up".format(interface=interface))
    ...
    def match_vf_value(self, value, vf_mac):
        ...
        vfs = self._ssh_helper.get_virtual_devices(value)
        log.info("vfs: %s", vfs)
