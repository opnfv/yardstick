...
class SriovStandaloneContext(StandaloneContext):
    ...
    def add_sriov_interface(index, vf_pci, vf_mac, xml):
        ...
        addr.set('slot', "{0}".format(pci_address.slot))
        addr.set('type', "pci")
    ...
    def _enable_interfaces(self):
        node, nic_details = self.get_nic_details()
        keys = 'interface', 'vf_pci', 'vf_macs'
        for index, (interface, vf_pci, vf_mac) in enumerate(make_dict_iter(nic_details, keys)):
            self.add_sriov_interface(index, vf_pci, vf_mac, self.cfg_file)
    ...
    def match_vf_value(self, value, vf_mac):
        ...
        log.info("vfs: %s", vfs)
        pci_0_bus = PciAddress.parse_address(str(value)).bus
        try:
            vf_value = next(v for k, v in vfs.items() if pci_0_bus == PciAddress(str(k)).bus)
        except StopIteration:
            vf_match = {"mac": vf_mac}
        else:
            vf_match = {"mac": vf_mac, "vf_pci": str(vf_value)}
