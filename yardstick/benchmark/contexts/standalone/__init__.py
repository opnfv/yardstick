...
class StandaloneContext(Context):
    ...
    def _make_vm_template(self):
        ...
        vcpus = self.nodes[0].get("vcpus", self.DEFAULT_VCPUS)
    ...
    def _start_vm(self, xml_path):
        out = self.ssh_helper.execute("virsh list --name | grep -i vm1")[1]
        if out == "vm1":
    ...
    def _get_server(self, attr_name):
        ...
        matching_node_iter = (n for n in self.nodes if n["name"] == node_name)
        try:
            # A clone is created in order to avoid affecting the
            # original one.
            node = deepcopy(next(matching_node_iter))
        except StopIteration:
            return None

        try:
            duplicate = next(matching_node_iter)
        except StopIteration:
            pass
        else:
