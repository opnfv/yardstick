...
class OvsdpdkStandalone(StandaloneContext):
    ...
    def _install_required_libraries(self):
        ...
        for cmd in cmd_list:
            out = self._ssh_helper.execute(cmd)[1]
            log.debug(out)
    ...
    def _setup_ovs(self, vpcis):
        ...
        for vpci in vpcis:
            self._ssh_helper.execute(nic_bind_cmd.format(vpci))
    ...
    def _add_oflows(self):
        self._ssh_helper.execute("ovs-ofctl del-flows br0")
        for flow in self.nodes[0]["flow"]:
            self._ssh_helper.execute(flow)
            self._ssh_helper.execute("ovs-ofctl dump-flows br0")
            self._ssh_helper.execute("ovs-vsctl set Interface dpdk0 options:n_rxq=4")
            self._ssh_helper.execute("ovs-vsctl set Interface dpdk1 options:n_rxq=4")

    def _setup_context(self):
        node0, nic_details = self.get_nic_details()
        self._setup_ovs(self.nodes[0]["phy_ports"])
        self._start_ovs_serverswitch()
        time.sleep(5)
        self._setup_ovs_bridge()
        self._add_oflows()
        self.setup_ovs_context(node0['phy_ports'], nic_details, node0['phy_driver'])
