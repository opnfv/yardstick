...
class StandaloneContextHelper(object):
    ...
    def install_req_libs(connection, extra_pkgs=None):
        ...
        pkgs = ["qemu-kvm", "libvirt-bin", "bridge-utils", "numactl", "fping", "libguestfs-tools",
                "genisoimage"]
    ...
    def check_update_key(cls, connection, node, xml_str, key_filename):
        ...
        if key_filename:
            m = re.search(
                r'<disk device="disk".*?<source file="((/[\w.-]+)+)"\s*/>', xml_str, re.S)
            ...
            if m:
                ...
                add_key_cmd = ('virt-sysprep -a %s --ssh-inject %s:file:%s' % (vm_image,
                                                                               vm_user,
                                                                               dst_pubkey_file))
                LOG.debug("Update key command: %s", add_key_cmd)
                status, _, error = connection.execute(add_key_cmd)
