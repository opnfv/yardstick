...
def add_ovs_interface(vpath, port_num, vpci, vports_mac, xml):
    ...
    root = ET.parse(xml)
    pci_address = utils.PciAddress(vpci.strip())
    ...
    _add_interface_address(interface, pci_address)
...
def add_sriov_interfaces(vm_pci, vf_pci, vf_mac, xml):
    ...
    pci_address = utils.PciAddress(vf_pci.strip())
    _add_interface_address(addr, pci_address)

    pci_vm_address = utils.PciAddress(vm_pci.strip())
    _add_interface_address(interface, pci_vm_address)
...
def get_virtual_devices(connection, pci):
    ...
    pattern = "PCI_SLOT_NAME=({})".format(utils.PciAddress.PCI_PATTERN_STR)
