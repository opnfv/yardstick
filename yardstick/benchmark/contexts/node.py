...
class NodeContext(Context):
    ...
    def init(self, attrs):
        ...
        self.controller.extend([node for node in cfg["nodes"]
                                if node["role"] == "Controller"])
        self.compute.extend([node for node in cfg["nodes"]
                             if node["role"] == "Compute"])
        self.baremetal.extend([node for node in cfg["nodes"]
                               if node["role"] == "Baremetal"])
        LOG.debug("Nodes: %r", self.nodes)
        LOG.debug("Controller: %r", self.controller)
        LOG.debug("Compute: %r", self.compute)
        LOG.debug("BareMetal: %r", self.baremetal)
