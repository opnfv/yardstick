...
class HeatContext(Context):
    ...
    def init(self, attrs):
        ...
        # try looking for external private key
        self.key_filename = attrs.get("key_filename", None)
        if self.key_filename is not None:
            # Disable key file generation if an external private key has been provided
    ...
    def _get_server(self, attr_name):
        ...
        if self.yardstick_gen_key_file:
            ...
            if not os.path.exists(self.key_filename):
                LOG.error("The key_filename provided %s does not exist !", self.key_filename)
            else:
                try:
                    pkey = open(self.key_filename, 'r').read().decode('utf-8')
                except IOError:
                    LOG.error("The file provided in key_filename %s is unreadable", self.key_filename)
