...
class CreateSubnet(base.Scenario):
    ...
    def run(self, *args):
        ...
        subnet_id = op_utils.create_neutron_subnet(
            self.shade_client, network_name_or_id=self.network_name_or_id,
            cidr=self.cidr, ip_version=self.ip_version,
            enable_dhcp=self.enable_dhcp, subnet_name=self.subnet_name,
            tenant_id=self.tenant_id, allocation_pools=self.allocation_pools,
