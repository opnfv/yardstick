...
class CreateServer(base.Scenario):
    ...
    def __init__(self, scenario_cfg, context_cfg):
        ...
        self.flavor = self.options.get("flavor")
        self.auto_ip = self.options.get("auto_ips", True)
        ...
        self.volume_size = self.options.get("volume_size", '50')
        ...
        self.network_id = self.options.get("network_id")
    ...
    def run(self, result):
        ...
        server = openstack_utils.create_instance_and_wait_for_active(
            self.shade_client, name=self.name, image=self.image,
            flavor=self.flavor, auto_ip=self.auto_ip, ips=self.ips,
