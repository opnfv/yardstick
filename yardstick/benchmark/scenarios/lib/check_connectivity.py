...
class CheckConnectivity(base.Scenario):
    ...
    def run(self, result):     # pragma: no cover
        ...
        if bool(status) != bool(conn_status):
            LOG.info("%s ===> %s connectivity check passed!" % (self.source_ip_addr, self.dest_ip_addr))
            result['Check_Connectivity'] = 'PASS'
        else:
            LOG.info("%s ===> %s connectivity check failed!" % (self.source_ip_addr, self.dest_ip_addr))
