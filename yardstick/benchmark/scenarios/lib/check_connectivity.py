...
class CheckConnectivity(base.Scenario):
    ...
    def __init__(self, scenario_cfg, context_cfg):
        ...
        try:
            ...
            self.ssh_key = self.options.get("ssh_key", None)
        ...
        try:
            ...
            if "sla" in self.scenario_cfg:
                ...
                if status != 0:
                    ...
                    assert conn_status, "%s =/=> %s" % (self.source_ip_addr, self.dest_ip_addr)
        except AssertionError:
            result.update({"Check_Connectivity": 'FAIL'})
        else:
            result.update({"Check_Connectivity": 'PASS'})
