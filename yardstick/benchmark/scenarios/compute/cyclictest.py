...
class Cyclictest(base.Scenario):
    """Execute cyclictest benchmark on host
  ...
  Parameters
    ...
    def run(self, args):
        ...
        args = "-a %s -i %s -p %s -l %s -t %s -h %s %s" \
               % (affinity, interval, priority, loops,
                  threads, histogram, default_args)
        cmd = "sudo bash cyclictest_benchmark.sh %s" % (args)
        ...
        if "sla" in args:
            sla_max_min_latency = int(args['sla']['max_min_latency'])
            min_latency = data['min']
            assert min_latency <= sla_max_min_latency, "min latency %f > " \
                "sla:max_min_latency(%f)" % (min_latency, sla_max_min_latency)

            sla_max_avg_latency = int(args['sla']['max_avg_latency'])
            avg_latency = data['avg']
            assert avg_latency <= sla_max_avg_latency, "avg latency %f > " \
                "sla:max_avg_latency(%f)" % (avg_latency, sla_max_avg_latency)

            sla_max_max_latency = int(args['sla']['max_max_latency'])
            max_latency = data['max']
            assert max_latency <= sla_max_max_latency, "max latency %f > " \
                "sla:max_max_latency(%f)" % (max_latency, sla_max_max_latency)
