...
class QemuMigrate(base.Scenario):
    ...
    def run(self, result):
        ...
        if "sla" in self.scenario_cfg:
            sla_error = ""
            sla_max_totaltime = int(self.scenario_cfg['sla']['max_totaltime'])
            totaltime = result["total_time"]
            if totaltime > sla_max_totaltime:
                sla_error += "Total time %f < " \
                             "sla:max_totaltime %f)" % (totaltime, sla_max_totaltime)
            assert sla_error == "", sla_error
            sla_max_downtime = float(self.scenario_cfg['sla']['max_downtime'])
            downtime = result["down_time"]
            if downtime > sla_max_downtime:
                sla_error += "downtime %f < " \
                             "sla:max_downtime %f)" % (downtime, sla_max_downtime)
            assert sla_error == "", sla_error
            sla_max_setuptime = int(self.scenario_cfg['sla']['max_setuptime'])
            setuptime = result["setup_time"]
            if setuptime > sla_max_setuptime:
                sla_error += "setuptime %d < " \
                             "sla:max_setuptime %d)" % (setuptime, sla_max_setuptime)
            assert sla_error == "", sla_error
