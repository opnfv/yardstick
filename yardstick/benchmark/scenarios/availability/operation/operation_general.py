...
from yardstick.benchmark.scenarios.availability.util import buildshellparams,execute_shell_command
        ...
        if "action_parameter" in self._config:
            if self.connection:
                ...
                exit_status, stdout = execute_shell_command(
                    "/bin/bash {0} {1}".format(self.action_script,
                        self.action_param))
        else:
            if self.connection:
                with open(self.action_script, "r") as stdin_file:
    ...
    def rollback(self):
        if "rollback_parameter" in self._config:
            if self.connection:
                ...
                exit_status, stdout = execute_shell_command(
                    "/bin/bash {0} {1}".format(self.rollback_script,
                        self.rollback_param))
        else:
            if self.connection:
                with open(self.rollback_script, "r") as stdin_file:
