...
class Sfc(base.Scenario):
    ...
    def setup(self):
        ...
        self.server.wait(timeout=600)
        cmd_SF1 = "nohup python vxlan_tool.py -i eth0 -d forward -v off -f -b 80 &"
        ...
        self.server.wait(timeout=600)
        cmd_SF2 = "nohup python vxlan_tool.py -i eth0 -d forward -v off -f -b 22 &"
    ...
    def run(self, result):
        ...
        cmd_client = "ssh root@" + target_ip
        result = self.client.execute(cmd_client)
        if "password" in result[1]:
            LOG.debug("SSH WORKED!!!!")
