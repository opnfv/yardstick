...
class NetperfNode(base.Scenario):
  ...
  Parameters
    ...
    def setup(self):
        ...
        # copy script to host
        self.remote_target_script = os.path.join('$HOME', os.path.basename(self.target_script))
        with open(self.target_script, "r") as target_script_file:
            self.client.run('cat > "{0}" && chmod +x "{0}"'.format(self.remote_target_script),
                            stdin=target_script_file)
