...
class VsperfDPDK(base.Scenario):
  ...
  Parameters:
    ...
    def setup(self):
        ...
        self.tg_port1_nw = vsperf.get('name', 'demo') + "-" + context_number + "-" + \
            self.scenario_cfg['options'].get('trafficgen_port1_nw', 'test2')
        self.tg_port2_nw = vsperf.get('name', 'demo') + "-" + context_number + "-" + \
    ...
    def dpdk_setup(self):
        ...
        cmd = "sshpass -p yardstick ssh-copy-id -o StrictHostKeyChecking=no root@%s -p 22" % \
            (self.moongen_host_ip)
    ...
    def run(self, result):
        ...
        additional_params = 'TRAFFIC={"traffic_type":"%s", "multistream":%d, ' \
                            '"l2":{"srcmac":"{\'%s\',\'%s\'}", "dstmac":"{\'%s\',\'%s\'}"}, "vlan":{"enabled":"True", "id":"{%d,%d}"}}' \
