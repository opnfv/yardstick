...
class Ping6(base.Scenario):  # pragma: no cover
    ...
    def _ssh_host(self, node_name):
        ...
        LOG.debug("node: %s, user: %s, host: %s", node, host_user, host_ip)
    ...
    def setup(self):
        ...
        if 'odl' in sdn:
            ...
            cmd = "sudo bash setup.sh %s" % self.os_cred
    ...
    def run(self, result):
        ...
        cmd = "sudo bash ping6.sh %s %s" % (self.os_cred, cmd_args)
        LOG.debug("Executing command: %s", cmd)
        status, stdout, stderr = self.client.execute(cmd)
    ...
    def teardown(self):
        ...
        cmd = "sudo bash teardown.sh %s" % self.os_cred
        status, stdout, stderr = self.client.execute(cmd)
