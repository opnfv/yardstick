...
class DPDKPktgen(base.Scenario):
  ...
  Parameters
    ...
    duration - duration of the test
        ...
        default: 20
    """
    __scenario_type__ = "DPDKPktgen"

    TARGET_SCRIPT = 'dpdk_pktgen_benchmark.bash'
    ...
    def run(self, result):
        ...
        sc_ipaddr = self.context_cfg["host"].get('private_ip')
	...
	cmd = "chmod +x ~/pktgen.sh"
	status, stdout, stderr = self.client.execute(cmd)

        cmd = "sudo bash pktgen.sh %s %s %s %s %s " \
            % (ipaddr, self.number_of_ports, packetsize, duration, sc_ipaddr)
        LOG.debug("Executing command: %s", cmd)
        status, stdout, stderr = self.client.execute(cmd)
