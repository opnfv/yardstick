...
class Pktgen(base.Scenario):
    ...
    def _iptables_setup(self):
        ...
        cmd = "sudo iptables -F; sudo iptables -A INPUT -p udp --dport 1000:%s -j DROP" \
    ...
    def _iptables_get_result(self):
        ...
        cmd = "sudo iptables -L INPUT -vnx | awk '/dpts:1000:%s/ {{printf \"%%s\", $1}}'" \
            % (1000 + self.number_of_ports)
        LOG.debug("Executing command: %s", cmd)
    ...
    def run(self, args):
        ...
        if "sla" in args:
            ...
            assert ppm <= sla_max_ppm, "ppm %d > sla_max_ppm %d" % (ppm, sla_max_ppm)
