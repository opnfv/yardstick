...
class NetworkServiceTestCase(base.Scenario):
    ...
    def _render_topology(self):
        ttopology = self._get_topology()
        ttopology_args = self.scenario_cfg.get('extra_args', {})
        ttopolgy_data = {
            'extra_args': ttopology_args
        }
        topology_yaml = vnfdgen.generate_vnfd(ttopology, ttopolgy_data)
