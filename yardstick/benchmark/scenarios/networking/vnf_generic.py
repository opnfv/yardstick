...
class NetworkServiceTestCase(base.Scenario):
    ...
    def _get_traffic_profile(cls, scenario_cfg, context_cfg):
        ...
        try:
            ...
            raw_traffic_profile = yaml.load(traffic_profile_tpl)
            traffic_mapping = raw_traffic_profile["traffic_mapping"]
        except KeyError:
            private = {}
            public = {}
        else:
            nodes = context_cfg["nodes"]
            public_host, public_if = traffic_mapping["public"].split(".")
            private_host, private_if = traffic_mapping["private"].split(".")
            public = nodes[public_host]["interfaces"][public_if]
            private = nodes[private_host]["interfaces"][private_if]
    ...
    def setup(self):
        ...
        try:
            ...
            LOG.error("Terminating all VNFs")
            for vnf in self.vnfs:
                vnf.terminate()
            raise
