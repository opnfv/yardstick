...
class MoongenTestPMD(base.Scenario):
  ...
  Parameters:
    ...
    def generate_config_file(self, frame_size, traffic_type, multistream,
        ...
        #    out_file.write("nrFlows = %d,\n" % (multistream))
        #    out_file.write("runBidirec = %s,\n" % (runBidirec))
        #    out_file.write("frameSize = %d,\n" % (frame_size))
        #    out_file.write("srcMacs = {\'%s\',\'%s\'},\n"
        #                   % (self.moongen_port1_mac,
        #                      self.moongen_port2_mac))
        #    out_file.write("dstMacs = {\'%s\',\'%s\'},\n"
        #                   % (self.tgen_port1_mac,
        #                      self.tgen_port2_mac))
        #    out_file.write("vlanIds = {%d,%d},\n" % (int(tg_port1_vlan),
        #                                             int(tg_port2_vlan)))
        #    out_file.write("searchRunTime = %d,\n" % (SearchRuntime))
        #    out_file.write("validationRunTime = %d,\n" % (SearchRuntime))
        #    out_file.write("acceptableLossPct = %s,\n" % (Package_Loss))
        #    out_file.write("ports = {0,1},\n")

        #    out_file.write("}" + "\n")
        #self.server.execute("rm -f result")
        print("########################################")
        print(Package_Loss)
        print(type(Package_Loss))
        out_text = """\
VSPERF {
testType = 'throughput',
nrFlows = %d,
...
""" % (multistream, runBidirec, frame_size, self.moongen_port1_mac,
    ...
    def run(self, result):
        ...
        tg_port1_vlan = json.loads(output).get("provider:segmentation_id")

        cmd = "openstack network show %s --format json -c " \
              "provider:segmentation_id" % (self.tg_port2_nw)
        LOG.debug("Executing command: %s", cmd)
        output = subprocess.check_output(cmd, shell=True)
        tg_port2_vlan = json.loads(output).get("provider:segmentation_id")
