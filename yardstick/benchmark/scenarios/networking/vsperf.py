...
class Vsperf(base.Scenario):
  ...
  Parameters:
    ...
    def setup(self):
        ...
        if self.br_ex and self.tg_port1:
            self._exec_command(OVS_VSCTL_CMD + ['add-port', self.br_ex,
                                                self.tg_port1])
        if self.br_ex and self.tg_port2:
            self._exec_command(OVS_VSCTL_CMD + ['add-port', self.br_ex,
                                                self.tg_port2])
        ...
        if self.setup_script:
            self._exec_command([self.setup_script, 'setup'])
    ...
    def teardown(self):
        ...
        if self.br_ex and self.tg_port1:
            self._exec_command(OVS_VSCTL_CMD + ['del-port', self.br_ex,
                                                self.tg_port1])
        if self.br_ex and self.tg_port2:
            self._exec_command(OVS_VSCTL_CMD + ['del-port', self.br_ex,
                                                self.tg_port2])
        ...
        # execute external setup script
        if self.setup_script:
            self._exec_command([self.setup_script, 'teardown'])

        self.setup_done = False

    @staticmethod
    def _exec_command(cmd):
        """execution of command defined by cmd list; Command is executed
           by subprocess call() method. All logs and errors will print
           given cmd as a list for easier debugging. RuntimeError
           is raised in case of command failure, i.e. non zero exit code.
        """
        LOG.info("Execute \"%s\"", cmd)
        if subprocess.call(cmd):
            raise RuntimeError('Execution of %s has failed.' % (cmd))
