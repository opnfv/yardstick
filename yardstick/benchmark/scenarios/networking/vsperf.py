...
class Vsperf(base.Scenario):
  ...
  Parameters:
    ...
    def __init__(self, scenario_cfg, context_cfg):
        ...
        self.test_params = self.scenario_cfg['options'].get('test-params', None)
    ...
    def run(self, result):
        ...
        """
        def add_test_params(options, option, default_value, alias=None):
        ...
        # escape quotes and double quotes in test_params
        test_params = filter(None, test_params)
        for i in range(len(test_params)):
            test_params[i] = test_params[i].replace('"', '\\"')
            test_params[i] = test_params[i].replace("'", "\\'")
