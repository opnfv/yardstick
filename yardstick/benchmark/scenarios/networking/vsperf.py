...
class Vsperf(base.Scenario):
  ...
  Parameters:
    ...
    duration - sets duration for which traffic will be generated
        type:    int
        default: 30
    bidirectional - speficies if traffic will be uni (False) or bi-directional (True)
        type:    string
        default: False
    ...
    rfc2544_trials - the number of trials performed for each packet size
        type:    string
        default: NA
    multistream - the number of simulated streems
        type:    string
        default: 0 (disabled)
    stream_type - specifies network layer used for multistream simulation
        ...
        type:    string
        default: "L4"
    conf-file - path to the vsperf configuration file, which will be uploaded to VM
        type:   string
        default: NA
    setup-script - path to the setup script, which will be executed during setup and
        teardown phases
        type:   string
        default: NA
    """
    ...
    def __init__(self, scenario_cfg, context_cfg):
        ...
        self.client = None
        self.tg_port1 = self.scenario_cfg['options'].get('trafficgen_port1', None)
        self.tg_port2 = self.scenario_cfg['options'].get('trafficgen_port2', None)
        self.br_ex = self.scenario_cfg['options'].get('external_bridge', 'br-ex')
        self.vsperf_conf = os.path.expanduser(
            self.scenario_cfg['options'].get('conf-file', Vsperf.VSPERF_CONF))
        self.setup_script = self.scenario_cfg['options'].get('setup-script', None)
    ...
    def setup(self):
        ...
        # add IXIA interfaces to the external bridge
        if self.tg_port1:
        ...
        if self.setup_script:
            cmd = "sudo %s setup" % (self.setup_script)
            LOG.info("Execute setup script %s setup", cmd)
    ...
    def run(self, result):
        ...
        if 'sla' in self.scenario_cfg and 'metrics' in self.scenario_cfg['sla']:
    ...
    def teardown(self):
        ...
        # remove IXIA interfaces from the external bridge
        if self.tg_port1:
        ...
        # execute external setup script
        if self.setup_script:
            cmd = "sudo %s teardown" % (self.setup_script)
            LOG.info("Execute setup script %s teardown", cmd)
            subprocess.call(cmd, shell=True)
