  ...
  Command line options:
    ...
    def run(self, result):
        ...
        #           time.sleep(int(esti_time)/2)
            ...
            result_res = requests.get('http://%s:5000/api/v1.0/jobs?type=metadata&id=%s' %
                                      (self.target, job_id))
            result_res_content = jsonutils.loads(
                result_res.content)
            if 'steady_state' in result_res_content['report']['details']:
                steady_state = result_res_content['report']['details']['steady_state'].values()[0]
                LOG.info("Job %s completed with steady state %s" % (job_id,steady_state))

            result_res = requests.get('http://%s:5000/api/v1.0/jobs?type=status&id=%s' %
                                      (self.target, job_id))
            result_res_content = jsonutils.loads(
                result_res.content)
