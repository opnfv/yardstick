  ...
  Command line options:
    ...
    def setup(self):
        ...
        if setup_res.status_code != 200:
            LOG.error("Failed to create stack. {0}: {1}"
                               .format(setup_res.status_code, setup_res.content))
            raise RuntimeError("Failed to create stack. {0}: {1}"
                               .format(setup_res.status_code, setup_res.content))
    ...
    def run(self, result):
        ...
        if job_res.status_code != 200:
            LOG.error("Failed to start job. {0}: {1}"
                               .format(job_res.status_code, job_res.content))
            raise RuntimeError("Failed to start job. {0}: {1}"
                               .format(job_res.status_code, job_res.content))
    ...
    def initialize_disks(self):
        ...
        if job_res.status_code != 200:
            LOG.error("Failed to start initialization job, error message: {0}: {1}"
                      .format(job_res.status_code, job_res.content))
            raise RuntimeError("Failed to start initialization job, error message: {0}: {1}"
                               .format(job_res.status_code, job_res.content))
        elif job_res.status_code == 200:
            job_res_content = jsonutils.loads(job_res.content)
            job_id = job_res_content["job_id"]
