...
class StorPerf(base.scenario):
  ...
  Command line options:
    ...
    def setup(self):
        ...
        setup_res = requests.post('http://StorPerf:5000/api/v1.0/\
                                  configurations', json=env_args)
    ...
    def _query_state(self, job_id):
        """Query the status of the supplied job_id and report on metrics"""
        LOG.debug("Fetching report for %s..." % (job_id,))
        report_res = requests.post('http://StorPerf:5000/api/v1.0/jobs?id=%s' %
                                   (job_id,))
    ...
    def run(self, result):
        ...
        job_res = requests.post('http://StorPerf:5000/api/v1.0/jobs',
                                json=job_args)
        ...
        LOG.debug("Started job id: %s..." % job_id)

        while not self.job_completed:
            est_time = self._query_state(job_id)
            time.sleep(est_time)

        # result.update(self._start_job())

    def teardown(self):
        """Reset the environment configrations back to default"""
        teardown_res = requests.delete('http://StorPerf:5000/api/v1.0/\
                                       configurations')
