...
class RunnerProducer(producer.MessagingProducer):
    ...
    def __init__(self, _id):
        super(RunnerProducer, self).__init__(messaging.TOPIC_RUNNER,
                                             _id=_id)

    def start_injection(self, version=1, data=None):
        data = {} if not data else data
        self.send_message(
            messaging.RUNNER_METHOD_START_INJECTION,
            payloads.RunnerPayload(version=version, data=data))

    def stop_injection(self, version=1, data=None):
        data = {} if not data else data
        self.send_message(
            messaging.RUNNER_METHOD_STOP_INJECTION,
            payloads.RunnerPayload(version=version, data=data))
