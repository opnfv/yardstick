...
def _worker_process(queue, cls, method_name, scenario_cfg,
    ...
    LOG.info("worker START, timeout %d sec, class %s", timeout, cls)
    ...
    while True:
        ...
        # Have to search through all the VNF KPIs
            ...
            kpi.get('done') for kpi in data.values() if isinstance(kpi, dict))

        if done or (errors and sla_action is None) or \
                (time.time() - start > timeout or aborted.is_set()):
            LOG.info("worker END")
            break
