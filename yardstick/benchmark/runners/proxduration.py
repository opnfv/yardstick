...
def _worker_process(queue, cls, method_name, scenario_cfg,
    ...
    requested_interval = interval = runner_cfg.get("interval", 20)
    ...
    while True:
        ...
        if sampled:
            ...
            for k, v in data.iteritems():
               LOG.info ("Sample Time%s -Item  %s -Content %s", time.time(), k, v)

        sequence += 1

        if (errors and sla_action is None):
            LOG.info ("Exit on Errors or SLA action ")
            break
        elif time.time() > timeout:
            LOG.info("Exit on Timeout ....")
            break
        elif aborted.is_set():
            LOG.info("Aborted ..")
            break
