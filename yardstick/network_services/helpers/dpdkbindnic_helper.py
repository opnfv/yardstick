...
class DpdkInterface(object):
    ...
    def _detect_socket(self, netdev):
        try:
...
class DpdkNode(object):
    ...
    def __init__(self, node_key, node_dict, timeout=120):
        ...
        interfaces = self.node_dict["interfaces"]
        try:
            ...
            errors = (name for name, intf in interfaces.items() if 'local_mac' not in intf)
    ...
    @property
    def interface_missing_iter(self):
        return ((name, intf.missing_fields) for name, intf in self.dpdk_interfaces.items())

    @property
    def interface_missing_map(self):
        return dict(self.interface_missing_iter)
    ...
    def check(self):
        ...
        try:
            missing_fields_set = set(chain.from_iterable(self.interface_missing_iter))

            # if we are only missing driver then maybe we can get kernel module
            # this requires vpci
            if "driver" in missing_fields_set:
            ...
            if missing_fields_set.difference({'driver'}):
                ...
                if set(chain.from_iterable(self.interface_missing_iter)):
            ...
            errors = ("{} missing: {}".format(name, ", ".join(missing_fields)) for
                      name, missing_fields in self.interface_missing_iter if missing_fields)
            errors = "\n".join(errors)
            if errors:
