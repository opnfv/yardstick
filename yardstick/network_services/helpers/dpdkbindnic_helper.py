...
class DpdkNode(object):
    ...
    def interface_missing_iter(self):
        return chain.from_iterable(self.interface_missing_map.values())
    ...
    def check(self):
        ...
        try:
            missing_fields_set = set(self.interface_missing_iter)
            ...
            if missing_fields_set.difference({'driver'}):
                ...
                if set(self.interface_missing_iter):
            ...
            errors = ("{} missing: {}".format(name, ", ".join(missing_fields)) for
                      name, missing_fields in self.interface_missing_map.items() if missing_fields)
            errors = "\n".join(errors)
            if errors:
...
"""
    ...
    def dpdk_devbind(self):
        if self._dpdk_devbind is None:
            self._dpdk_devbind = self.ssh_helper.provision_tool(tool_path=self.bin_path,
