...
class DpdkInterface(object):
    ...
    @staticmethod
    def iter_key_translate(data, key_map):
        return ((dest_key, data[src_key]) for dest_key, src_key in key_map.items())
        ...
        try:
            for netdev in self.dpdk_node.netdevs.values():
                if netdev['address'].lower() == self.mac_lower:
                    update_iter = self.iter_key_translate(netdev, TOPOLOGY_UPDATABLE_MAP)
                    self.interface.update(update_iter)
    ...
    def check(self):
        ...
        # rebind
        self.dpdk_node.force_dpdk_rebind()
...
class DpdkNode(object):
    ...
    @property
    def netdevs(self):
        self._netdevs = self.probe_devices()
    ...
    def probe_devices(self):
        ...
        try:
            ...
            netdevs = self.probe_netdevs()

        except IncorrectNodeSetup:
            raise
...
"""
    ...
    def force_dpdk_rebind(self):
        ...
        for driver, pcis in self.used_drivers.items():
            # only rebind igb_uio
            if driver == self.dpdk_driver:
                for pci in pcis:
                    # messy
    ...
    def get_real_kernel_driver(self, pci):
        ...
        if match:
            real_driver = self.real_kernel_interface_driver_map[pci] = match.group(1)
            return real_driver

        match = self.VIRTIO_DRIVER_RE.search(out)
        if match:
            real_driver = self.real_kernel_interface_driver_map[pci] = self.VIRTIO_DRIVER
            return real_driver
    ...
    def get_real_kernel_drivers(self):
        self.real_kernel_interface_driver_map = {
            self.get_real_kernel_driver(pci) for pci in self.interface_driver_map
        }
