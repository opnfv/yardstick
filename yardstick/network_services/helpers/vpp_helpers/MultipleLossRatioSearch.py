...
# yardstick comment: this is a modified copy of csit/resources/libraries/python/MLRsearch/MultipleLossRatioSearch.py
...
from yardstick.network_services.helpers.vpp_helpers.AbstractSearchAlgorithm import \
...
class MultipleLossRatioSearch(AbstractSearchAlgorithm):
    ...
    def narrow_down_ndr_and_pdr(
        ...
        MultipleLossRatioSearch.display_single_bound(result_samples,
                                                     'NDR_LOWER',
                                                     result.ndr_interval.measured_low.transmit_rate,
                                                     self.pkt_size,
                                                     result.ndr_interval.measured_low.latency)
        MultipleLossRatioSearch.display_single_bound(result_samples,
                                                     'NDR_UPPER',
                                                     result.ndr_interval.measured_high.transmit_rate,
                                                     self.pkt_size)
        MultipleLossRatioSearch.display_single_bound(result_samples,
                                                     'PDR_LOWER',
                                                     result.pdr_interval.measured_low.transmit_rate,
                                                     self.pkt_size,
                                                     result.pdr_interval.measured_low.latency)
        MultipleLossRatioSearch.display_single_bound(result_samples,
                                                     'PDR_UPPER',
                                                     result.pdr_interval.measured_high.transmit_rate,
                                                     self.pkt_size)
    ...
    def perform_additional_measurements_based_on_ndrpdr_result(self, result):
        ...
        for i in range(0, 1):
            ...
            LOGGING.info(
                'Traffic loss occurred: {0}'.format(self.measurer.loss))

    @staticmethod
    def display_single_bound(result_samples, result_type, rate_total, pkt_size,
                             latency=None):
        bandwidth_total = rate_total * (pkt_size + 20) * 8 / (10 ** 9)
