...
class MultiPortConfig(object):
    def __init__(
            ...
            worker_config='1C/1T', lb_config='SW', nfv_type='baremetal',
            socket=0):
        ...
        self.nfv_type = nfv_type
        self.is_openstack = False
        self.socket = socket
        if nfv_type == 'openstack' and worker_threads != 1:
            self.nfv_type = 'ovs'
        if nfv_type == 'openstack':
            self.init_eal()
            self.is_openstack = True

    def init_eal(self):
        with open('/etc/yardstick/nodes/pod.yaml') as fh:
            yaml_data = yaml.safe_load(fh.read())
        interfaces = yaml_data['nodes'][1]['interfaces']
        vpci = [interfaces[item]['vpci'] for item in interfaces]
    ...
    def generate_lb_to_port_pair_mapping(self):
        ...
        for i in xrange(self.lb_count):
            self.lb_to_port_pair_mapping[i + 1] = port_pair_count
        for i in xrange(len(self.port_pair_list) % self.lb_count):
    ...
    def generate_arpicmp_data(self):
        ...
        if self.nfv_type == 'ovs':
            self.init_ovs = False
            port_list = self.port_pair_list[:self.lb_to_port_pair_mapping[1]]
            pktq_out = [float(x[-1]) + float(
                0.1 * self.worker_threads) for x in list(
                itertools.chain(*port_list))]
            arpicmp_data['pktq_out'] = ' '.join(
                ['TXQ' + str(x) for x in pktq_out])
            self.ovs_pktq_out = pktq_out
        self.swq += self.lb_count
        if self.nfv_type == 'openstack':
    ...
    def generate_final_txrx_data(self):
        if self.nfv_type in ('ovs', 'openstack'):
            start = 0
            end = self.worker_threads
        else:
            start = 1
            end = self.worker_threads + 1
    ...
    def generate_initial_txrx_data(self):
        if self.nfv_type == 'ovs':
            pktq = [float(x[-1]) + float(0.1 * i) for i in range(
                ...
                itertools.chain(*self.port_pairs))]
            if not self.init_ovs:
                pktq.extend(self.ovs_pktq_out)
                self.init_ovs = True
        elif self.nfv_type == 'openstack':
            pktq = [float(x[-1]) + float(0.1 * i) for i in range(
                0, self.worker_threads) for x in list(
                itertools.chain(*self.port_pairs))]
        else:
            pktq = [float(x[-1]) for port_pair in self.port_pairs for x in
                    port_pair]
        txrx_data = {
    ...
    def generate_config(self):
        self.get_port_pairs()
        if self.vnf_type == 'VPE':
            self.generate_vpe_config_data()
        else:
        ...
        if self.is_openstack:
            ...
            mode = 'w'
        with open(self.tmp_file, mode) as tfh:
