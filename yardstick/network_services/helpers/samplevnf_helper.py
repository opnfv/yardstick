...
class MultiPortConfig(object):
    ...
    def init_write_parser_template(self):
        for section in self.read_parser:
            section_type = self.read_parser.section_get(section, 'type', None)
            if section_type in {'ARPICMP', 'MASTER'}:
                self.write_parser.sections.append(copy.deepcopy(section))
    ...
    def generate_config_data(self):
        ...
        self.write_parser.update_section_data(self.arpicmp_tpl, arpicmp_data)
        ...
        import pydevd
        pydevd.settrace('127.0.0.1', port=5000, stdoutToServer=True, stderrToServer=True)
        new_pipelines = []
        ...
        for lb in self.lb_to_port_pair_mapping:
            ...
            if self.lb_config == 'SW':
                core, rxrx_data = self.generate_initial_txrx_data()
                self.start_core += 1
