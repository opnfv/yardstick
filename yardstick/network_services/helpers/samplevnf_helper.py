...
class MultiPortConfig(object):
    ...
    def generate_arpicmp_data(self):
        swq_str = self.make_range_str('SWQ{}', self.swq, offset=self.lb_count)
        ...
        self.pktq_out_os = swq_str.split(' ')
        if self.lb_config == "HW":
            arpicmp_data['pktq_in'] = swq_str
            self.swq = 0
        return arpicmp_data
    ...
    def generate_arp_config(self):
        ...
        return ''.join(('p {3} arpadd {0} {1} {2}\n'.format(*values) for values in arp_config))
    ...
    def generate_arp_config6(self):
        ...
        return ''.join(('p {3} arpadd {0} {1} {2}\n'.format(*values) for values in arp_config6))
