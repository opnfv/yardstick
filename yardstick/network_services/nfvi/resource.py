...
from yardstick.benchmark.contexts.heat import HeatContext
...
class ResourceProfile(object):
    ...
    def __init__(self, mgmt, port_names=None, plugins=None,
        ...
        self.reset_rabbitmq = reset_rabbitmq
    ...
    def make_from_node(cls, node, timeout):
        ...
        if node.get("ctx_type") == HeatContext.__context_type__:
    ...
    def _start_rabbitmq(self, connection):
        if self.reset_rabbitmq:
            self._reset_rabbitmq(connection)
        else:
            admin_exist = False
            exit_status, stdout, stderr = connection.execute("sudo rabbitmqctl list_users")
            if exit_status == 0:
