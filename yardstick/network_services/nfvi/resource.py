...
class ResourceProfile(object):
    ...
    def _start_collectd(self, connection, bin_path):
        ...
        connection.execute("ulimit -n 1000000  && sudo %s" % collectd_path, timeout=self.timeout)
        connection.execute("sudo bash -c 'ulimit -n 1000000 ; %s'" % collectd_path, timeout=self.timeout)
        LOG.debug("Done")
