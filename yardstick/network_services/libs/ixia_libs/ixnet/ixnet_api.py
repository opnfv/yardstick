...
class IxNextgen(object):  # pragma: no cover
    ...
    def update_frame(self, traffic, duration):
        ...
        for traffic_param in traffic.values():
            ...
            rate = traffic_param.get('rate')
            rate_unit = (
                'framesPerSecond' if traffic_param.get('rate_unit') ==
    ...
    def update_ip_packet(self, traffic):
        ...
        for traffic_param in traffic.values():
            ...
            count = traffic_param['outer_l3'].get('count', 1)
            srcip = traffic_param['outer_l3'].get('srcip')
            dstip = traffic_param['outer_l3'].get('dstip')
            srcseed = traffic_param['outer_l3'].get('srcseed', 1)
            dstseed = traffic_param['outer_l3'].get('dstseed', 1)
            srcmask = traffic_param['outer_l3'].get('srcmask') \
                      or IP_VERSION_4_MASK
            dstmask = traffic_param['outer_l3'].get('dstmask') \
                      or IP_VERSION_4_MASK
            if srcip:
    ...
    def update_l4(self, traffic):
        ...
        for traffic_param in traffic.values():
            ...
            if proto:
                ...
                count = traffic_param['outer_l4'].get('count', 1)
                seed = traffic_param['outer_l4'].get('seed', 1)

                srcport = traffic_param['outer_l4'].get('srcport')
                srcmask = traffic_param['outer_l4'].get('srcportmask')

                dstport = traffic_param['outer_l4'].get('dstport')
                dstmask = traffic_param['outer_l4'].get('dstportmask')
