			...
			for index, ti in enumerate(tis):
				...
				for idx, stream in enumerate(items):
					ti = ixNet.remapIds(ti)[0]
	                        	ixNet.setMultiAttribute(ti+'/{0}:'.format(stream)+str(index)+'/transmissionControl',
        	                        	'-type', '{0}'.format(param['traffic_type']),
	                	                '-duration', '{0}'.format(param['duration']))

                        		ixNet.setMultiAttribute(ti+'/{0}:'.format(stream)+str(index)+'/frameRate', '-rate', param['iload'])
		                        if param['outer_l2']['framesPerSecond'] == True:
        	        	        	ixNet.setMultiAttribute(ti+'/{0}:'.format(stream)+str(index)+'/frameRate', '-type', 'framesPerSecond')

					for key, value in param['outer_l2']['framesize'].items():
					        pairs = []
						if value != '0':
					        	weightedPairs.append(key.replace('B','').replace('b',''))
						        weightedPairs.append(value)
						        pairs.append(key.replace('B','').replace('b',''))
						        pairs.append(key.replace('B','').replace('b',''))
						        pairs.append(value)
					        	weightedRangePairs.append(pairs)
					ixNet.setMultiAttribute(ti+'/{0}:'.format(stream)+str(index)+'/frameSize', 
