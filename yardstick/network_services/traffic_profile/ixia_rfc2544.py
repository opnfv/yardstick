...
class IXIARFC2544Profile(TrexProfile):
    def _get_ixia_traffic_profile(self, profile_data, mac={}, xfile=None):
	result = {}
        STATIC_TRAFFIC = {}
        if xfile: 
		...
		for key, value in profile_data[traffickey].items():
        		...
        		traffic['bidir'] = False
			traffic['outer_l2']['srcmac'] = mac["src_mac_{}".format(traffic['id'])]
			traffic['outer_l2']['dstmac'] = mac["dst_mac_{}".format(traffic['id'])]
		        # outer_l3
			if "outer_l3v6" in value.keys():
    ...
    def start_ixia_latency(self, traffic_generator, ixia_obj, mac={}, xfile=None):
	traffic = self._get_ixia_traffic_profile(self.full_profile, mac)
	self._ixia_traffic_generate(traffic_generator, traffic, ixia_obj, xfile)

    def get_drop_percentage(self, traffic_generator, samples, tol_min, tolerance, ixia_obj, mac={}, xfile=None):
        status = 'Running'
        ...
        rx_throughput = sum([samples[iface]['RxThroughput'] for iface in samples])
        tx_throughput = sum([samples[iface]['TxThroughput'] for iface in samples])
