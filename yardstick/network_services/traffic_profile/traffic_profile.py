...
class TrexProfile(TrafficProfile):
    ...
    def __init__(self, yaml_data):
        ...
        self.flows = self.params['traffic_profile'].get('flow_number', 100)
    ...
    def set_src_mac(self, src_mac):
        ...
        max_value = src_macs[1] if len(src_macs) == 2 else src_macs[0]
        if len(src_macs) == 1:
            src_mac = min_value
    ...
    def set_dst_mac(self, dst_mac):
        ...
        max_value = dst_macs[1] if len(dst_macs) == 2 else dst_macs[0]
        if len(dst_macs) == 1:
            dst_mac = min_value
    ...
    def generate_imix_data(self, packet_definition):
        ...
        if imix:
            ...
            if imix_sum > 100:
                logging.debug("Error in IMIX data")
                raise SystemExit(1)
            elif imix_sum < 100:
    ...
    def _get_start_end_ipv6(cls, start_ip, end_ip):
        try:
            ...
            if ((hi1 << 64) | lo1) > ((hi2 << 64) | lo2):
                logging.debug("IPv6: start_ip is greater then end_ip")
                raise SystemExit(2)
            max_p1 = abs(int(lo1) - int(lo2))
            base_p1 = lo1
        except Exception as ex_error:
            logging.error(ex_error)
            raise SystemExit(2)
        else:
