...
class PortPgIDMap(object):
    ...
    "pg_ig" is the identification STL library gives to each stream. In the
    ...
    def increase_pg_id(self, port=None):
        ...
        port_map = self._port_pg_id_map.get(port)
        if not port_map:
            self._port_pg_id_map[port] = []
            port_map = self._port_pg_id_map[port]
        self._pg_id += 1
        port_map.append(self._pg_id)
        return self._pg_id
...
class RFC2544Profile(TrexProfile):
    ...
    def get_drop_percentage(self, samples, tolerance_low, tolerance_high,
        ...
        for sample in samples:
            tx_rate_fps += sum(port['tx_throughput_fps'] for port in sample)
            rx_rate_fps += sum(port['rx_throughput_fps'] for port in sample)
        tx_rate_fps /= len(samples)
        rx_rate_fps /= len(samples)

        # TODO(esm): RFC2544 doesn't tolerate packet loss, why do we?
        out_packets = sum(port['opackets'] for port in samples[-1])
        in_packets = sum(port['ipackets'] for port in samples[-1])
        drop_percent = 100.0
