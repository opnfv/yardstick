...
class ProxBinSearchProfile(ProxProfile):
    ...
    def run_test_with_pkt_size(self, traffic_gen, pkt_size, duration):
        ...
        single_samples["test_duration"] = traffic_gen.scenario_helper.scenario_cfg["runner"]["duration"]
        single_samples["test_precision"] = self.params["traffic_profile"]["test_precision"]
        single_samples["tolerated_loss"] = self.params["traffic_profile"]["tolerated_loss"]
        single_samples["duration"] = duration
        ...
        for test_value in self.bounds_iterator(LOG):
            ...
            if result.success:
                ...
                success_samples = {}
                for key in samples:
                    success_samples["Success_" + key] = samples[key]
                self.queue.put(success_samples)

                # Store Actual throughput for result samples
                result_samples["Result_Actual_throughput"] = success_samples["Success_RxThroughput"]
            else:
            ...
            if theor_max_thruput < samples["TxThroughput"]:
                theor_max_sample = {}
                theor_max_thruput = theor_max_sample["theor_max_throughput"] = samples["TxThroughput"]
                self.queue.put(theor_max_sample)
