...
class ProxBinSearchProfile(ProxProfile):
    ...
    def run_test_with_pkt_size(self, traffic_gen, pkt_size, duration):
        ...
        ok_retry = traffic_gen.scenario_helper.scenario_cfg["runner"].setdefault("confirmation", 0)
            ...
            while (pos_retry <= ok_retry) and (neg_retry <= ok_retry):
                ...
                if total_retry > (ok_retry * 3):
                    ...
                    if pos_retry < ok_retry:
                        ...
                        LOG.info(">>>##>>Collect SUCCESS TG KPIs %s %s", datetime.datetime.now(), success_samples)
                ...
                self.queue.put(samples, True, constants.QUEUE_PUT_TIMEOUT)
