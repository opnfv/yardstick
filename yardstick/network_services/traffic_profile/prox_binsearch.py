        ...
        for test_value in self.bounds_iterator(LOG):
            ...
            while (pos_retry <= ok_retry) and (neg_retry <= ok_retry):
                ...
                if (total_retry > (ok_retry * 3)) and (ok_retry is not 0):
                    LOG.debug("Failure.!! .. RETRY EXCEEDED ... decrease lower bound")
                    ...
                    if result.success:
                        if (pos_retry < ok_retry) and (ok_retry is not 0):
                            neg_retry = 0
                            LOG.debug("Success! ... confirm retry")
                            ...
                            LOG.debug(">>>##>>Collect SUCCESS TG KPIs %s %s",
                        ...
                        if (neg_retry < ok_retry) and (ok_retry is not 0):
                            ...
                            pos_retry = 0
                            LOG.debug("failure! ... confirm retry")
                        else:
