...
class ProxProfile(TrafficProfile):
    ...
    def __init__(self, tp_config):
        ...
        self.tolerated_loss = float(
            self.prox_config.get('tolerated_loss', 0.0))
    ...
    def execute(self, traffic_generator):
        ...
        self.run_test_with_pkt_size(traffic_generator, self.queue, pkt_size,
                                    duration)

    @staticmethod
    def get_samples(traffic_gen, pkt_size, mpps, pps, successfull_pkt_loss,
                    latency):
        samples = {
            "Throughput": mpps,
            "DropPackets": successfull_pkt_loss,
            "CurrentDropPackets": successfull_pkt_loss,
            "TxThroughput": (pps / 1000000.0),
            "RxThroughput": mpps,
            "PktSize": pkt_size,
        }
        if len(latency) >= 3:
            samples["LatencyMin"] = latency[0]
            samples["LatencyMax"] = latency[1]
            samples["LatencyAvg"] = latency[2]
        ...
        for vpci_idx, intf in enumerate(traffic_gen.vpci_if_name_ascending):
            ...
            xe_port = traffic_gen.tester.port_stats([vpci_idx])
            samples[name] = {"in_packets": xe_port[6],
