...
class ProxProfile(TrafficProfile):
    ...
    def make_profile_helper(self, traffic_gen):
        self.traffic_config = traffic_gen.scenario_helper.scenario_cfg["options"].get("traffic_config", {})
        ...
        self.current_lower = self.lower_bound = float(self.traffic_config.get('lower_bound', self.lower_bound))
        self.current_upper = self.upper_bound = float(self.traffic_config.get('upper_bound', self.upper_bound))
