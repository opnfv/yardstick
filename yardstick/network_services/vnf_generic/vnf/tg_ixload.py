...
class IxLoadResourceHelper(ClientResourceHelper):
    ...
    def setup(self):
        self.resource_file_name = str(self.scenario_helper['ixia_profile'])
...
class IxLoadTrafficGen(SampleVNFTrafficGen):
    ...
    def run_traffic(self, traffic_profile):
        ports = []
        card = None
        for interface in self.scenario_helper.interfaces:
        ...
        ixia_config = self.scenario_helper.mgmt_interface["tg-config"]
        ixload_config = IXLOAD_CONFIG_TEMPLATE % (
            ixia_config["ixchassis"], ports, card,
            self.scenario_helper.mgmt_interface["ip"], self.ssh_helper.bin_path,
