...
class ProxApproxVnf(SampleVNF):
    ...
    def collect_kpi(self):
        ...
        all_ports = self.vnf_execute('multi_port_stats', range(port_count))
        tx_total = rx_total = 0
        ...
        try:
            ...
            LOG.error("Zero Division Error on curr_packet_in")
            curr_packets_in = 0
        try:
            curr_packets_fwd = int(((tx_total - self.prev_packets_sent) * self.tsc_hz)
                                / (tsc - self.prev_tsc) * port_count)
        except ZeroDivisionError:
            LOG.error("Zero Division Error on curr_packet_fwd")
            curr_packets_fwd = 0
