...
CORES = ['0', '1', '2']
WAIT_TIME = 20
VPE_CONFIG = "/tmp/vpe_config"
VPE_SCRIPT = "/tmp/vpe_script"
APP_NAME = "vPE_vnf"
...
class VpeApproxVnf(GenericVNF):
    ...
    def instantiate(self, scenario_cfg, context_cfg):
        ...
        self.connection.execute("sudo pkill %s", APP_NAME)
        dpdk_nic_bind = \
            provision_tool(self.connection,
    ...
    def _run_vpe(self, filewrapper, vnf_cfg):
        ...
        vpe_config = open(VPE_CONFIG).read()
        self._provide_config_file(posixpath.basename(VPE_CONFIG),
                                  vpe_config, vpe_vars)

        vpe_script = vpe_conf.generate_vpe_script(interfaces)
        self._provide_config_file(posixpath.basename(VPE_SCRIPT),
                                  vpe_script, vpe_vars)

        LOG.info("Provision and start the vPE")
        tool_path = provision_tool(self.connection,
                                   os.path.join(self.bin_path, APP_NAME))
        ports_len_hex = \
            hex(eval(
