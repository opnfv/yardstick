...
VPE_PIPELINE_COMMAND = '{tool_path} -p {ports_len_hex} -f {cfg_file} -s {script}'
CORES = ['0', '1', '2', '3', '4', '5']
WAIT_TIME = 20
...
class ConfigCreate(object):
    ...
    def vpe_upstream(self, vnf_cfg, index):
        ...
        for pipeline in parser.sections():
            for (k, v) in parser.items(pipeline):
                if k == "pktq_in":
                     if "RXQ" in v:
                        parser.set(pipeline, k, "RXQ%s.0" % self.priv_ports[index])
                     else:
                        parser.set(pipeline, k, self.get_sink_swq(parser, pipeline, k, index))
                if k == "pktq_out":
                     if "TXQ" in v:
                        parser.set(pipeline, k, "TXQ%s.0" % self.pub_ports[index])
                     else:
                        self.sw_q += 1
                        parser.set(pipeline, k, self.get_sink_swq(parser, pipeline, k, index))
    ...
    def get_firewall_script(self, pipeline, ip):
        ...
        for i in range(256):
            ip_addr[-2] = str(i)
            ip = '.'.join(ip_addr)
            fwl += 'p {0} firewall add priority 1 ipv4  {1} 24 0.0.0.0 0 0 65535 0 65535 6 0xFF port 0\n'.format(pipeline, ip)
    ...
    def get_route_script(self, pipe_line_id, ip, mac_addr):
        ...
        for i in range(0, 256, 8):
            ip_addr[-2] = str(i)
            ip = '.'.join(ip_addr)
            fwl += 'p {0} route add {1} 21 port 0 ether {2} mpls 0:{3}\n'.format(pipe_line_id, ip, mac_addr, i)
    ...
    def get_route_script2(self, pipe_line_id, ip, mac_addr):
        ...
        for i in range(0, 256):
            ip_addr[-2] = str(i)
            ip = '.'.join(ip_addr)
            fwl += 'p {0} route add {1} {2} port 0 ether {3} qinq 0 {4}\n'.format(pipe_line_id, ip, mask, mac_addr, i)
        fwl += 'p {0} route add default 1\n\n'.format(pipe_line_id)
        return fwl
    ...
    def generate_vpe_script(self, interfaces):
        ...
        for idx in range(0, len(self.priv_ports)):
            port0_ip = interfaces[self.priv_ports[idx]]["virtual-interface"]["local_ip"]
            port1_ip = interfaces[self.pub_ports[idx]]["virtual-interface"]["local_ip"]
            dst_port0_ip = interfaces[self.priv_ports[idx]]["virtual-interface"]["dst_ip"]
            dst_port1_ip = interfaces[self.pub_ports[idx]]["virtual-interface"]["dst_ip"]
            dst_port0_mac = interfaces[self.priv_ports[idx]]["virtual-interface"]["dst_mac"]
            dst_port1_mac = interfaces[self.pub_ports[idx]]["virtual-interface"]["dst_mac"]
            ...
            num_pipe += 1
            fwl += self.get_route_script2(num_pipe, dst_port0_ip, dst_port0_mac)
            num_pipe += 4
...
class VpeApproxVnf(GenericVNF):
    ...
    def instantiate(self, scenario_cfg, context_cfg):
        ...
        self.socket = \
            next((0 for v in interfaces
                  if v['virtual-interface']["vpci"][5] == "0"), 0)
    ...
    def _run_vpe(self, filewrapper, vnf_cfg):
        ...
        ports_len_hex = \
            hex(eval(
                '0b' + "".join([str(1) for x in xrange(len(self.my_ports))])))
