...
class VpeApproxSetupEnvHelper(DpdkVnfSetupEnvHelper):
    ...
    def _build_vnf_ports(self):
        tg_port_pairs = \
            MultiPortConfig.get_port_pairs(self.vnfd_helper.interfaces)[0]
        self.priv_ports = [int(x[0][2:]) for x in tg_port_pairs]
        self.pub_ports = [int(x[1][2:]) for x in tg_port_pairs]
