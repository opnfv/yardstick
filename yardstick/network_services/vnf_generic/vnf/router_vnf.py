...
class RouterVNF(SampleVNF):
    ...
    def __init__(self, name, vnfd, setup_env_helper_type=None, resource_helper_type=None):
        ...
        super(RouterVNF, self).__init__(name, vnfd, setup_env_helper_type,
    ...
    def instantiate(self, scenario_cfg, context_cfg):
        ...
        self._vnf_password = self.context_cfg["nodes"][self.name].get("password")
        # if in OpenStack context, wait for the VNF to start
        #if not self._vnf_password and self.name in scenario_cfg['options']:
        self._fallback_password = None
        if self._vnf_password is None:
            self._fallback_password = scenario_cfg['options'][self.name].get("password")
            if self._fallback_password is not None:
                LOG.debug("Using password set in scenario: %s",
                          self._fallback_password)
                self.ssh_helper.password = \
                  scenario_cfg['options'][self.name]['password']
    ...
    def configure_routes(self, scenario_cfg):
        ...
        ip_cmd_replace = 'sudo /sbin/ip addr replace %s/24 dev %s'
        ip_cmd_up = 'sudo /sbin/ip link set %s up'
        ip_cmd_flush = 'sudo /sbin/ip address flush dev %s'
        ...
        ip_cmd = 'sudo /sbin/ip neigh add %s lladdr %s dev %s nud perm'
        neighbor_count = 0
        ...
        self.arp_status = self.ssh_helper.execute("arp -a -n")
        LOG.debug('arp %s', self.arp_status)
        LOG.debug('neigh count: %s', neighbor_count)
        enable_forwarding = self.ssh_helper.execute("sudo sysctl net.ipv4.ip_forward=1")
