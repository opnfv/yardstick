...
class CgnaptApproxSetupEnvHelper(DpdkVnfSetupEnvHelper):
    ...
    def _generate_ip_from_pool(ip):
        ...
        iter1 = (str(n) for n in range(int(ip_parts[2]), stop=256))
    ...
    def _vnf_up_post(self):
        if self.vnfd_helper.options.get('napt', 'static') != 'static':
            return

        ip_iter = self._generate_ip_from_pool("152.16.40.10")
        gw_ips = self._get_cgnapt_config()
        if self.scenario_helper.vnf_cfg.get("lb_config", "SW") == 'HW':
            pipeline = self.HW_DEFAULT_CORE
            offset = 3
        else:
            pipeline = self.SW_DEFAULT_CORE - 1
            offset = 0
