...
from yardstick.common.utils import open_relative_file
...
class LandslideTrafficGen(SampleVNFTrafficGen):
    ...
    def __init__(self, name, vnfd, task_id, setup_env_helper_type=None, resource_helper_type=None):
        ...
        super(LandslideTrafficGen, self).__init__(name, vnfd, task_id, setup_env_helper_type,
    ...
    def instantiate(self, scenario_cfg, context_cfg):
        ...
        # Create SUTs
        [self.resource_helper.create_suts(x['suts']) for x in self.vnfd_helper['config']]
    ...
    def _update_session_suts(suts, testcase):
        """ Create SUT entry. Update related EPC block data in session profile. """
        for _sut in suts:
            # Update session profile EPC element info with SUT info from pod file
            tc_role = testcase['parameters'].get(_sut['role'])
            if tc_role:
                _param = {}
                if tc_role['class'] == 'Sut':
                    _param['name'] = _sut['name']
                elif tc_role['class'] == 'TestNode':
                    _param.update({x: _sut[x] for x in {'ip', 'phy', 'nextHop'}
                                   if x in _sut and _sut[x]})
                testcase['parameters'][_sut['role']].update(_param)
            else:
                LOG.info('Unexpected SUT role in pod file: "{}".'.format(_sut['role']))
        return testcase

    def _update_session_test_servers(self, test_server, _tsgroup_index):
        """ Update tsId, reservations and re-resolved ARP in session profile """
    ...
    def _update_session_tc_params(self, tc_options, testcase):
    ...
    def _load_session_profile(self):
        ...
        with open_relative_file(self.scenario_helper.scenario_cfg['session_profile'],
                                self.scenario_helper.task_path) as stream:
            self.session_profile = yaml_load(stream)
        ...
        session_tests_num = [xx for x in self.session_profile['tsGroups'] for xx in x['testCases']]
        # Create a set containing number of list elements in each structure
        _config_files_blocks_num = [
            len(x) for x in (self.vnfd_helper['config'],  # test_servers and suts info
                             session_tests_num,
        ...
        for _config, tc_options in zip(self.vnfd_helper['config'],  # test servers and SUTS
                                       self.scenario_helper.all_options['test_cases']):  # testcase

            _ts_config = _config['test_server']

            # Calculate test group and test case indexes based on test server name
            ...
            _testcase = self.session_profile['tsGroups'][_tsgroup_idx]['testCases'][_testcase_idx]

            if _testcase['type'] != _ts_config['role']:
                raise RuntimeError('Test type mismatch in TC#{} of test server {}'.format(
            ...
            self.session_profile['tsGroups'][_tsgroup_idx]['testCases'][_testcase_idx].update(
...
class LandslideResourceHelper(ClientResourceHelper):
    ...
    def abort_running_tests(self, timeout=60, delay=5):
        ...
        while time.time() < _start_time + timeout:
            run_tests_states = {x['id']: x['testStateOrStep'] for x in self.get_running_tests()}
            if not set(run_tests_states.values()).difference({'COMPLETE', 'COMPLETE_ERROR'}):
                break
            else:
                [self.stop_running_tests(running_test_id=_id, force=True) for _id, _state in
                 run_tests_states.items()
                 if 'COMPLETE' not in _state]
            time.sleep(delay)
        else:
            raise RuntimeError('Some test runs not stopped during {} seconds'.format(timeout))
    ...
    def _build_url(self, resource, action=None):
        ...
        # Concatenate actions
        _action = ''.join(['?{}={}'.format(k, v) for k, v in action.items()]) if action else ''
    ...
    def get_response_params(self, method, resource, params=None):
        ...
        # If URL last part and top dict key do not match, expect dict(), not a list()
        k, v = list(_response_json.items())[0]
        if k != url_last_part:
            v = [v]  # v: list(dict(str: str))
        # Extract only params if it is specified, or whole list of dicts (without top level key)
        for x in v:
    ...
    def _create_user(self, auth, level=1):
        ...
        :returns int: user id
        """
        # Set expiration date as two years since account creation date
        _exp_date = time.strftime('{}/%m/%d %H:%M %Z'.format(time.gmtime().tm_year + 2))
        _username = auth['user']
        _fields = {"contactInformation": "", "expiresOn": _exp_date, "fullName": "Test User",
                   "isActive": "true", "level": level, "password": auth['password'],
                   "username": _username}
        _response = self.exec_rest_request('post', self.users_uri, json_data=_fields)
        _resp_json = _response.json()
        if _response.status_code == self.REST_STATUS_CODES['CREATED']:
            # New user created
            _id = _resp_json['id']
            LOG.info("New user created: username='%s', id='%s'", _username, _id)
        elif _resp_json.get('apiCode') == self.REST_API_CODES['NOT MODIFIED']:
            # User already exists
            LOG.info("Account '%s' already exists.", _username)
            # Update expiration date and return user id
            # WORKAROUND: due to HAA-3119
            # _id = self._modify_user(_username, {"expiresOn": _exp_date})
            _id = self._get_users(_username)[0]['id']
        else:
            raise RestApiError('Error during new user "{}" creation'.format(_username))
        return _id
    ...
    def _modify_user(self, username, fields):
        ...
        :returns str: user id
        """
        _response = self.exec_rest_request('post', self.users_uri, action={'username': username},
                                           json_data=fields)
        LOG.info("User account '%s' modified: '%s'", username, _response.json())
    ...
    def _delete_user(self, username):
        ...
        """
        self.exec_rest_request('delete', self.users_uri, action={'username': username})
    ...
    def _get_users(self, username=None):
        ...
        _res = [u for u in _response if u['username'] == username] if username else _response
        return _res

    def exec_rest_request(self, method, resource, action=None, json_data=None, logs=True):
        ...
        :type json_data: dict
        :param logs: paremeter specifying if debug logs for specific call would be displayed
        ...
        if method == 'post' and not action:
            if not (json_data and isinstance(json_data, Mapping)):
                raise ValueError('JSON data missing in {} request'.format(_method))

        r = getattr(self.session, method)(self._build_url(resource, action), json=json_data)
        if logs:
            LOG.debug("RC: %s | Request: %s | URL: %s", r.status_code, method, r.request.url)
    ...
    def connect(self):
        ...
        self.session.auth = (tas_info['super-user'], tas_info['super-user-password'])
    ...
    def create_suts(self, suts):
        ...
        for _sut in suts:
            sut_entry = {k: v for k, v in _sut.iteritems() if k not in {'phy', 'nextHop', 'role'}}
            _response = self.exec_rest_request(
                'post', self.suts_uri, json_data=sut_entry, logs=False)
            if _response.status_code != self.REST_STATUS_CODES['CREATED']:  # Failed to create
                LOG.info(_response.reason)
    ...
    def get_suts(self, suts_id=None):
        if suts_id:
            _suts = self.exec_rest_request('get', '{}/{}'.format(self.suts_uri, suts_id)).json()
        else:
            _suts = self.get_response_params('get', self.suts_uri)

        # LOG.info("SUTs configured on TAS Manager: %s", _suts)
        return _suts
    ...
    def configure_sut(self, sut_name, json_data):
        ...
        _response = self.exec_rest_request('post',
                                          ...
                                          json_data=json_data)
        if _response.status_code not in {self.REST_STATUS_CODES[x] for x in {'OK', 'NO CHANGE'}}:
    ...
    def delete_suts(self, suts_ids=None):
        ...
        for _id in suts_ids:
            self.exec_rest_request('delete', '{}/{}'.format(self.suts_uri, _id))
            LOG.info("\tDone for SUT id: %s", _id)

    def _check_test_servers_state(self, test_servers_ids=None, delay=10, timeout=300):
        LOG.info("Waiting for related test servers state change to READY...")
        # Wait on state change
        _start_time = time.time()
        while time.time() - _start_time < timeout:
            ts_ids_not_ready = {x['id'] for x in self.get_test_servers(test_servers_ids)
                                if x['state'] != 'READY'}
            if ts_ids_not_ready == set():
                break
            time.sleep(delay)
        else:
            raise RuntimeError('Test servers not in READY state after {} seconds.'.format(timeout))

    def create_test_servers(self, test_servers):
        """ Create test servers

        :param test_servers: input data for test servers creation
                             mandatory fields: managementIp optional fields: name
        :type test_servers: list(dict)
        """
        _ts_ids = []
        for _ts in test_servers:
            _msg = 'Created test server "{name}"'
            _ts_ids.append(self._tcl.create_test_server(_ts))
            if _ts.get('thread_model'):
                _msg += ' in mode: "{thread_model}"'
                LOG.info(_msg.format(**_ts))
    ...
    def configure_test_servers(self, action, json_data=None, test_server_ids=None):
        ...
        for _id in test_server_ids:
            self.exec_rest_request('post', '{}/{}'.format(self.test_serv_uri, _id),
                                   action=action, json_data=json_data)
            LOG.info("Test server (id: %s) configuration done: %s", _id, action)
    ...
    def delete_test_servers(self, test_servers_ids=None):
        ...
        for _ts in self.get_test_servers(test_servers_ids):
            self.exec_rest_request('delete', '{}/{}'.format(self.test_serv_uri, _ts['id']))
    ...
    def get_test_session(self, test_session_name=None):
        if test_session_name:
            uri = 'libraries/{}/{}/{}'.format(self._user_id, self.test_session_uri,
                                              test_session_name)
        else:
    ...
    def configure_test_session(self, template_name, test_session):
        ...
        return self.exec_rest_request(
            ...
            resource='{}/{}'.format(self.user_lib_uri.format(self._user_id), template_name))
    ...
    def get_running_tests(self, running_test_id=None):
        ...
        # If no run_id specified, skip top level key in response dict. Else return JSON as list
    ...
    def _running_tests_action(self, running_test_id, action, json_data=None):
        if not json_data:
            json_data = {}
        # Supported actions: 'stop', 'abort', 'continue', 'update', 'sendTcCommand', 'sendOdc'
        _res_name = '{}/{}'.format(self.running_tests_uri, running_test_id)
        self.exec_rest_request('post', _res_name, {'action': action}, json_data)
        LOG.debug("Executed action: '%s' on running test id: %s", action, running_test_id)

    def stop_running_tests(self, running_test_id, json_data=None, force=False):
        _action = 'abort' if force else 'stop'
        self._running_tests_action(running_test_id, _action, json_data=json_data)
        LOG.info('Performed action: "%s" to test run with id: %s', _action, running_test_id)

    def check_running_test_state(self, run_id):
        r = self.exec_rest_request('get', '{}/{}'.format(self.running_tests_uri, run_id))
        return r.json().get("testStateOrStep")

    def get_running_tests_results(self, run_id):
        _res = self.exec_rest_request('get', '{}/{}/{}'.format(self.running_tests_uri, run_id,
                                                               'measurements')).json()
        return _res
    ...
    def _write_results(self, results):
        ...
        # Avoid parsing 'tab' dict key initially (it could be missing or contain empty dict)
        if not _res_tabs:
            return
        ...
        for _tab, _kpis in _res_tabs.items():
            for _kpi, _value in _kpis.items():
                ...
                try:
                    ...
                    # Remove comma as decimal separator and/or measure units, e.g. "us"
                    flat_kpis_dict[_key] = float(_value.split(' ')[0].replace(',', ''))
                except ValueError:  # E.g. if KPI represents datetime
                    pass
            # Use current time for timestamps
        LOG.info("Polling test results of test run id: {}. Elapsed time: {} seconds".format(
            self.run_id, _elapsed_time))
...
class LandslideTclClient(object):
    ...
    def __init__(self, tcl_handler, ts_context):
        ...
        # Set of test types names expected in session profile, test case and pod files
        self._tc_types = {"SGW_Nodal", "SGW_Node", "MME_Nodal", "PGW_Node", "PCRF_Node"}
    ...
    def connect(self, url, auth):
        ...
        res = self._tcl.execute("ls::login {} {} {}".format(url, auth[0], auth[1]))
        if res != 'java0x2':
            raise LandslideTclException("connect: login failed ='{}'.".format(res))
        self._library_id = self._tcl.execute(
            "ls::get [ls::query LibraryInfo -userLibraryName {}] -Id".format(auth[0]))
        self._basic_library_id = self._get_library_id('Basic')
        self._url = url
        self._user = auth[0]
        LOG.debug("connect: user='%s' me='%s' basic='%s'", self._user, self._library_id,
    ...
    def create_test_server(self, test_server):
        ...
        try:
            ...
            ts_id = self._tcl.execute('ls::perform AddTs -Name "{}" -Ip "{}"'.format(
                _ts_name, _ts_ip))
            try:
                int(ts_id)
            except ValueError:
                # Failed to create test server, e.g. limit reached
                raise RuntimeError('Failed to create test server: "{}". {}'.format(_ts_name,
                                                                                   ts_id))
        # Retrieve current TsInfo configuration, result stored in handle "ts"
        self._tcl.execute('set ts [ls::retrieve TsInfo -Name "{}"]'.format(_ts_name))

        # Set license ID, if it differs from current one, and update test server configuration
        ...
        # Skip below code modifying thread_model if it is not defined for this test server
        if _ts_thread_model:
            # Retrieve test server configuration, store it in handle "tsc"
            _cfguser_password = self._ts_context.vnfd_helper['mgmt-interface']['cfguser_password']
            self._tcl.execute(
                'set tsc [ls::perform RetrieveTsConfiguration -name "{}" {}]'.format(
                    _ts_name, _cfguser_password))
            # Configure ThreadModel of test server, if it differs from current one
            _model = _thread_model_map[_ts_thread_model]
            _curr_model = self._tcl.execute('ls::get $tsc -ThreadModel')
            if _curr_model != _model:
                self._tcl.execute('ls::config $tsc -ThreadModel "{}"'.format(_model))
                self._tcl.execute('ls::perform ApplyTsConfiguration $tsc {}'.format(
                    _cfguser_password))
        return ts_id
    ...
    def create_test_session(self, test_session):
        ...
        self._tcl.execute('ls::config $test_ -Library {} -Name "{}"'.format(self._library_id,
                                                                            test_session['name']))
        self._tcl.execute('ls::config $test_ -Description "{}"'.format(
            test_session['description']))
        if 'keywords' in test_session:
            self._tcl.execute('ls::config $test_ -Keywords "{}"'.format(test_session['keywords']))
        if 'duration' in test_session:
            self._tcl.execute('ls::config $test_ -Duration "{}"'.format(test_session['duration']))
    ...
    def create_dmf(self, dmf):
        ...
        self._tcl.execute('ls::config $dmf_ -Library {} -Name "{}"'.format(_lib_id,
                                                                           dmf['dmf']['name']))
            ...
            if isinstance(_param_value, dict):
                ...
                for _sub_param_key in _param_value:
                    ...
                    if isinstance(_sub_param_value, str):
                        _tcl_cmd += ' -{} "{}"'.format(_sub_param_key, _sub_param_value)
                    else:
                        _tcl_cmd += ' -{} {}'.format(_sub_param_key, _sub_param_value)
                ...
                if isinstance(_param_value, str):
                    self._tcl.execute('ls::config $dmf_ -{} "{}"'.format(_param_key,
                                                                         _param_value))
                else:
                    self._tcl.execute('ls::config $dmf_ -{} {}'.format(_param_key,
                                                                       _param_value))
    ...
    def _save_dmf(self):
        # Call 'Validate' before saving to set default values for missing parameters
        ...
        for _option_key in options:
            ...
            if _option_key == 'format':
                ...
                if _option_value == 'CSV':
                    _format = 1
                self._tcl.execute(
                    'ls::config $test_.ReportOptions -Format {} -Ts -3 -Tc -3'.format(_format))
            else:
                self._tcl.execute('ls::config $test_.ReportOptions -{} {}'.format(_option_key,
                                                                                  _option_value))
    ...
    def _configure_ts_group(self, ts_group, ts_group_index):
        try:
            ...
            raise RuntimeError('Test server name "{}" does not exist.'.format(ts_group['tsId']))
        if _ts_id not in self.ts_ids:
            self._tcl.execute('set tss_ [ls::create  TsGroup  -under $test_  -tsId {} ]'.format(
                _ts_id))
    ...
    def _configure_tc_type(self, tc, ts_group_index):
        if tc['type'] not in self._tc_types:
            raise RuntimeError('Test type %s not supported.', tc['type'])
        tc['type'] = tc['type'].replace('_', ' ')
        res = self._tcl.execute('set tc_ [ls::retrieve testcase -libraryId {0} "{1}"]'.format(
            self._basic_library_id, tc['type']))
        if 'Invalid' in res:
            raise RuntimeError('Test type %s not found in "Basic" library.', tc['type'])
        self._tcl.execute('ls::config $test_.TsGroup({}) -children-Tc $tc_'.format(
            ts_group_index))
        self._tcl.execute('ls::config $tc_ -Library {0} -Name "{1}"'.format(
            self._basic_library_id, tc['name']))
        self._tcl.execute('ls::config $tc_ -Description "{}"'.format(tc['type']))
        self._tcl.execute('ls::config $tc_ -Keywords "GTP LTE {}"'.format(tc['type']))
        if 'linked' in tc:
            self._tcl.execute('ls::config $tc_ -Linked {}'.format(tc['linked']))
        if 'AssociatedPhys' in tc:
            self._tcl.execute('ls::config $tc_ -AssociatedPhys "{}"'.format(tc['AssociatedPhys']))
        if 'parameters' in tc:
        ...
        for _param_key in sorted(params):
            ...
            if isinstance(_param_value, dict):
                ...
                if _param_value['class'] in self._class_param_config_handler:
                    self._class_param_config_handler[_param_value['class']](_param_key,
                                                                            _param_value)
            else:
                # Configure simple parameter
                self._tcl.execute('ls::create {} -under $p_ -Value "{}";'.format(_param_key,
                                                                                 _param_value))
    ...
    def _configure_array_param(self, name, params):
        ...
        for param in params['array']:
            self._tcl.execute('ls::create ArrayItem -under $p_.{} -Value "{}"'.format(name,
                                                                                      param))
    ...
    def _configure_sut_param(self, name, params):
        self._tcl.execute('ls::create -Sut-{} -under $p_ -Name "{}";'.format(name,
                                                                             params['name']))
    ...
    def _configure_dmf_param(self, name, params):
        ...
        for _flow_index, _flow in enumerate(params['mainflows']):
            ...
            self._tcl.execute('ls::perform AddDmfMainflow $p_.Dmf {} "{}"'.format(_lib_id,
                                                                                  _flow['name']))
            if params.get('instanceGroups'):
                ...
                for _key in ['mixType', 'rate']:
                    if _key in _instance_group:
                        self._tcl.execute('ls::config $p_.Dmf.InstanceGroup({}) -{} {}'.format(
                ...
                for _row_id, _row in enumerate(_instance_group.get('rows', [])):
                    self._tcl.execute(
                        ...
                        -PreferredTransport {} -RatingGroup {} -ServiceID {}'.format(
                            _flow_index, _row_id, _row['node'], _row['overridePort'],
                            _row['clientPort'], _row['context'], _row['role'],
                            _row['transport'], _row['ratingGroup'], _row['serviceId']))

    def _configure_reservation(self, reservation):
        _ts_id = self.resolve_test_server_name(reservation['tsId'])
        self._tcl.execute('set reservation_ [ls::create Reservation -under $test_]')
        self._tcl.execute('ls::config $reservation_ -TsIndex {} -TsId {} -TsName "{}"'
                          .format(reservation['tsIndex'], _ts_id, reservation['tsName']))
        for _subnet in reservation['phySubnets']:
            self._tcl.execute('set physubnet_ [ls::create PhySubnet -under $reservation_]')
            self._tcl.execute(
                'ls::config $physubnet_ -Name "{}" -Base "{}" -Mask "{}" -NumIps {}'.format(
                    _subnet['name'], _subnet['base'], _subnet['mask'], _subnet['numIps']))
    ...
    def _configure_preresolved_arp(self, pre_resolved_arp):
        ...
        for _entry in pre_resolved_arp:
            # TsGroup handle name should correspond with one in _configure_ts_group()
            self._tcl.execute('ls::create  PreResolvedArpAddress -under $tss_  -StartingAddress '
                              '"{StartingAddress}" -NumNodes {NumNodes}'.format(**_entry))
    ...
    def _get_library_id(self, library):
        _library_id = self._tcl.execute(
            "ls::get [ls::query LibraryInfo -systemLibraryName {}] -Id".format(library))
        try:
            int(_library_id)
            return _library_id
        except ValueError:
        ...
        _library_id = self._tcl.execute(
            "ls::get [ls::query LibraryInfo -userLibraryName {}] -Id".format(library))
        try:
            int(_library_id)
        except ValueError:
            LOG.error("_get_library_id: library='%s' not found.", library)
            raise LandslideTclException("_get_library_id: library='{}' not found.".format(
                library))
...
class LsTclHandler(object):
    ...
    def __init__(self):
        ...
        self._ls.tcl("ls::config ApiOptions -NoReturnSuccessResponseString '{}'".format(
            self.LS_OK))
