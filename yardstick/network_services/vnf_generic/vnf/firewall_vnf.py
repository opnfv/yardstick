...
class FirewallVNFDeployHelper(object):

    def __init__(self, vnfd_helper, ssh_helper):
        super(FirewallVNFDeployHelper, self).__init__()
        self.ssh_helper = ssh_helper
        self.vnfd_helper = vnfd_helper
    ...
    def deploy_vnfs(self, app_name):
        time.sleep(2)
#        exit_status = self.ssh_helper.execute("ifconfig")
#        LOG.debug("ifconfig status " + str(exit_status))
        # Configure IP of dtaplane ports
        # TODO:
#        exit_status = self.ssh_helper.execute("/sbin/ifconfig dp0s10 152.16.100.19/24 up")
#        exit_status = self.ssh_helper.execute("/sbin/ifconfig dp0s11 152.16.40.19/24 up")
        ...
        #done
#        arp1_status = self.ssh_helper.execute("arp -i dp0s10 -s 152.16.100.26 90:e2:ba:bf:8a:10")
#        LOG.debug("arp1" + str(arp1_status))

#        arp2_status = self.ssh_helper.execute("arp -i dp0s11 -s 152.16.40.26 90:e2:ba:bf:8a:40")
#        LOG.debug("arp2" + str(arp2_status))

#        arp_status = self.ssh_helper.execute("arp -a")
#        LOG.debug("arp" + str(arp2_status))
...
class FirewallVNF(GenericVNF):
    ...
    def __init__(self, name, vnfd, setup_env_helper_type=None, resource_helper_type=None):
        super(FirewallVNF, self).__init__(name, vnfd)
        self.bin_path = get_nsb_option('bin_path', '/root/dummy.py')
        ...
        self.deploy_helper = FirewallVNFDeployHelper(vnfd, self.ssh_helper)
    ...
    def instantiate(self, scenario_cfg, context_cfg):
        ...
        self.deploy_helper.deploy_vnfs("FirewallVNF")
    ...
    def collect_kpi(self):
        ...
        LOG.debug("%s collect KPIs %s", "FirewallVNF", result)
        return result


class FirewallVNFTrafficGen(GenericTrafficGen):
    ...
    def __init__(self, name, vnfd, setup_env_helper_type=None, resource_helper_type=None):
        super(FirewallVNFTrafficGen, self).__init__(name, vnfd)
