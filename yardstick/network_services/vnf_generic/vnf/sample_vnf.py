...
class VnfSshHelper(AutoConnectSSH):
    ...
    def copy(self):
        # our copy constructor is different from SSH classes, since we use node
        return self.get_class()(self.node, self.bin_path)

    def execute(self, cmd, *args, **kwargs):
        self._connect()
        return super(VnfSshHelper, self).execute(cmd, *args, **kwargs)

    def run(self, *args, **kwargs):
        self._connect()
        return super(VnfSshHelper, self).run(*args, **kwargs)

    def put(self, *args, **kwargs):
        self._connect()
        return super(VnfSshHelper, self).put(*args, **kwargs)

    def put_file_obj(self, *args, **kwargs):
        self._connect()
        return super(VnfSshHelper, self).put_file_obj(*args, **kwargs)
...
class SetupEnvHelper(object):
    ...
    CFG_CONFIG = "/tmp/sample_config"
    CFG_SCRIPT = "/tmp/sample_script"
    CORES = []
...
class DpdkVnfSetupEnvHelper(SetupEnvHelper):
    ...
    def _find_used_drivers(self):
        ...
        self.used_drivers = {
            ...
            if any(b.endswith(m0) for b in self.bound_pci)
        }
    ...
    def _setup_dpdk(self):
        ...
        status = self.ssh_helper.execute("which {} >/dev/null 2>&1".format(dpdk))[0]
        if status:
    ...
    def _setup_resources(self):
        ...
        return ResourceProfile(self.vnfd_helper.mgmt_interface, cores)
