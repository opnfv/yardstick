...
"""
    ...
    def _probe_missing_values(cls, netdevs, network):
        ...
        for netdev in netdevs.values():
            if netdev['address'].lower() != mac_lower:
                continue
            socket = cls._detect_socket(netdev)
            network.update({
    ...
    def _probe_netdevs(self, timeout=120):
        ...
        netdevs = self.parse_netdev_info(stdout)
    ...
    def _detect_and_bind_drivers(self):
        ...
        netdevs = self._probe_netdevs()
        for interface in interfaces:
            self._probe_missing_values(netdevs, interface)
            missing = self.TOPOLOGY_REQUIRED_KEYS.difference(interface)
            if missing:
