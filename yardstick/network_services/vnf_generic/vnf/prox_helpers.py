        ...
        for i, (_, section) in enumerate(sections):
            for index, (item_key, item_val) in enumerate(section):
                if item_val.startswith("@@dst_mac"):
                    ...
                    port_if = self.vnfd_helper.vdu[0]["external-interface"][tx_port_no]
                    mac = port_if["virtual-interface"]["dst_mac"]
                    mac = mac.replace(":", " ", 6)
                ...
                if item_key == "dst mac" and item_val.startswith("@@"):
                    ...
                    sections[i][SECTION_CONTENTS][index][1] = \
                        port_if["virtual-interface"]["dst_mac"]
                ...
                for index, (item_key, item_val) in enumerate(section):
                    try:
                        if item_key.startswith("dofile"):
                            ...
                            new_string = file_str[0] + '"' + \
                                self.additional_files[base_proxfile] + '"' + file_str[2]
    ...
    def build_config_file(self):
        ...
        try:
            ...
            for key_prox_file in proxfile_config_path:
                ...
                remote_proxfile = self.copy_to_target(key_prox_file, base_proxfile)
                self.additional_files[base_proxfile] = remote_proxfile
        except:
            pass
...
class ProxResourceHelper(ClientResourceHelper):
    ...
    def traffic_context(self, pkt_size, value):
        ...
        if self.get_test_type() == self.PROX_CORE_MPLS_TEST:
            self.tagged_cores, self.plain_cores = self.get_cores_mpls()
    ...
    def run_test(self, pkt_size, duration, value, tolerated_loss=0.0):
        ...
        samples = {}
        for iface in range(interface_count):
            port_rx_total, port_tx_total = self.sut.port_stats([iface])[6:8]
            name = self.vpci_if_name_ascending[iface]["name"]
            samples[name] = {"in_packets": port_rx_total,
    ...
    def get_test_type(self):
        test_type = None
        for i, (section_name, section) in enumerate(self.setup_helper.prox_config_dict):
...
#                   elif section[1][1] == PROX_CORE_BNG_TEST:
#                       test_type = PROX_CORE_BNG_TEST
        return test_type
    ...
    def get_cores_mpls(self, mode=PROX_CORE_GEN_MODE):
        ...
        for i, (section_name, section) in enumerate(self.setup_helper.prox_config_dict):
            if section_name.startswith("core"):
                for index, (item_key, item_value) in enumerate(section):
                    if item_key == "mode" and item_value == mode:
                        for index2, (item2_key, item2_value) in enumerate(section):
                            if item2_key == "name" and item2_value.startswith("tag"):
                                cores_tag = self.get_cpu_id_from_list(
                                    *self.get_core_socket(section_name))
                                cores_tagged.append(cores_tag)
                            elif item2_key == "name" and item2_value.startswith("udp"):
                                cores_udp = self.get_cpu_id_from_list(
                                    *self.get_core_socket(section_name))
                                cores_plain.append(cores_udp)
