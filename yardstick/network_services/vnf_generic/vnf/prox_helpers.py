...
class ProxSocketHelper(object):
    ...
    def multi_port_stats(self, ports):
        """get counter values from all ports port at once"""
            ...
            if len(ports) is len(ports_all_data):
                for port_data_str in ports_all_data:
                    ...
                    try:
                        ...
                        LOG.error("Unpacking data error %s", port_index, port_data_str)
        ...
        if retry_counter >= 10:
            LOG.error("Empty ... multi port packet")
            return False, tot_result
        else:
            LOG.info("Multi port packet ..OK.. %s", tot_result)
...
class ProxDataHelper(object):
    ...
    def totals_and_pps(self):
        if self._totals_and_pps is None:
            rx_total = tx_total = 0
            ok, all_ports = self.sut.multi_port_stats([self.vnfd_helper.port_num(port_name) for port_name in self.vnfd_helper.port_pairs.all_ports])
            if ok:
