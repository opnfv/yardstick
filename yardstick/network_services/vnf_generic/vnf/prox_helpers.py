        ...
        while len(ports) is not len(ports_all_data):
            ...
            if len(ports) is len(ports_all_data):
                for port_data_str in ports_all_data:
                    ...
                    if (len(tot_result[port_index]) is not 6) or tot_result[port_index][0] is not ports[port_index]:
                        ports_all_data = []
                        tot_result = []
                        port_index = 0
                        time.sleep(0.01)
                        LOG.error("Corrupted PACKET %s - retrying %s", port_data_str)
                        break
                    else:
                        port_index = port_index + 1
            else:
                LOG.error("Empty data - retry")
        return tot_result
