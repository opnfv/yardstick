...
class VppSetupEnvHelper(DpdkVnfSetupEnvHelper):
    ...
    def crypto_device_init(self, pci_addr, numvfs):
        ...
        self.dpdk_bind_helper.load_dpdk_driver('qat_dh895xcc')
        ...
        self.dpdk_bind_helper.bind(pci_addr, 'dh895xcc')
    ...
    def vpp_create_ipsec_tunnels(self, if1_ip_addr, if2_ip_addr, if_name,
        ...
        if n_connections <= n_tunnels:
            ...
            count = n_connections / n_tunnels
        addr_ip_i = int(ip_address(unicode(addrs_ip)))
        dst_start_ip = addr_ip_i

        tmp_fn = '/tmp/ipsec_create_tunnel_dut.config'
        ...
        with open(tmp_fn, 'w') as tmp_file:
        ...
        self.execute_script(tmp_fn, json_out=False, copy_on_execute=True)
        os.remove(tmp_fn)
        ...
        with open(tmp_fn, 'w') as tmp_file:
            for i in range(0, n_tunnels):
                if count > 1:
                    ...
                    dst_end_ip = ip_address(dst_start_ip + count - 1)
                    ips = [ip_address(ip) for ip in
                           [str(ip_address(dst_start_ip)), str(dst_end_ip)]]
                ...
                start_tunnel = start_tunnels_cmds.format(
                    raddr=str(ip_address(dst_start_ip)), mask=mask_length,
                    addr=if2_ip_addr,
                    i=i, count=count,
                    uifc=vpp_ifname)
    ...
    def vpp_interfaces_ready_wait(self, timeout=30):
        ...
        while not if_ready:
            ...
            if not not_ready:
                ...
                LOG.debug('Interfaces still in link-down state: {0}, '
    ...
    def update_vpp_interface_data(self):
        ...
        for interface in self.vnfd_helper.interfaces:
            ...
            if not interface_dict:
                LOG.debug(
                    'Interface {0} not found by MAC {1}'.format(interface,
                                                                if_mac))
                continue
        ...
        for interface in self.vnfd_helper.interfaces:
            ...
            if ret == 0:
                try:
                    ...
                    LOG.debug(
                        'Reading numa location failed for: {0}'.format(
                            interface["virtual-interface"]["vpci"]))
    ...
    def get_vpp_interface_by_mac(interfaces_list, mac_address):
        ...
        list_mac_address = VppSetupEnvHelper._convert_mac_to_number_list(
            mac_address)
        LOG.debug("MAC address {0} converted to list {1}."
...
class VatTerminal(object):
    ...
    def vat_terminal_exec_cmd(self, cmd):
        try:
            ...
            raise RuntimeError(
                "VPP not running on node. VAT command {0} execution failed".
                    format(cmd))
        if self.json:
            obj_start = out.find('{')
            obj_end = out.rfind('}')
