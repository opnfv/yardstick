...
class VcmtsPktgen(SampleVNFTrafficGen):
    ...
    def instantiate(self, scenario_cfg, context_cfg):
        ...
        if pktgen_values == None:
            LOG.exception("Error reading pktgen_values file provided (%s)",
        ...
        if self.pod_cfg == None:
            raise KeyError("Pktgen with id " + str(self.pktgen_id) + " not found")

        self.setup_helper.start_pktgen(self.pod_cfg)

    def run_traffic(self, traffic_profile):
        #print(json.dumps(traffic_profile, indent=4, sort_keys=True))
