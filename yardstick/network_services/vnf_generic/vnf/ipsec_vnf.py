...
from yardstick.network_services.vnf_generic.vnf.vpp_helpers import VppSetupEnvHelper
...
class VipsecApproxSetupEnvHelper(VppSetupEnvHelper):
    ...
    def _get_crypto_type(self):
        vnf_cfg = self.scenario_helper.options.get('vnf_config', self.DEFAULT_IPSEC_VNF_CFG)
    ...
    def setup_vnf_environment(self):
        resource = super(VipsecApproxSetupEnvHelper, self).setup_vnf_environment()
    ...
    def check_status(self):
        ...
        entries = re.split("\n+", stdout)
        tmp = [re.split("\s\s+", entry, 5) for entry in entries]
