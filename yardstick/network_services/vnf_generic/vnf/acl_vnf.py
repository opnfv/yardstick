...
class AclApproxVnf(GenericVNF):
    ...
    def __setup_hugepages(cls, connection):
        hugepages = \
            connection.execute(
                "sudo awk '/Hugepagesize/ { print $2$3 }' < /proc/meminfo")[1]
        hugepages = hugepages.rstrip()

        memory_path = \
            '/sys/kernel/mm/hugepages/hugepages-%s/nr_hugepages' % hugepages
        connection.execute("sudo awk -F: '{ print $1 }' < %s" % memory_path)
    ...
    def instantiate(self, scenario_cfg, context_cfg):
        ...
        rule_file = self.options[self.name]['rules']
    ...
    def _get_cpu_sibling_list(self, cores):
        try:
            ...
            for core in cores:
                ...
                cpuid = \
                    self.connection.execute(
                        "sudo awk -F: '{ print $1 }' < %s" % sys_cmd)[1]
                cpu_topo += \
    ...
    def collect_kpi(self):
        ...
        pattern = "ACL TOTAL:\stpkts_processed:\s(\d+),\spkts_drop:\s(\d+)," \
            "\spkts_received:\s(\d+),"
