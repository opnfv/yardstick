...
class TrexTrafficGen(GenericTrafficGen):
    ...
    def _connect_client(self):
        ...
        for idx in range(6):
            try:
                client.connect()
                LOG.info("Connecting to Trex... Attempt %s", idx)
                break
            except STLError:
                LOG.info("Failed connecting to Trex Traffic Generator")
                time.sleep(5)
        return client
    ...
    def _traffic_runner(self, traffic_profile, queue,
        ...
        while not terminated.value:
            ...
            for vpci_idx in range(len(self._vpci_ascending)):
                ...
                samples[name] = \
                    {"rx_throughput_fps": float(last_res[vpci_idx]["rx_pps"]),
                     "tx_throughput_fps": float(last_res[vpci_idx]["tx_pps"]),
                     "rx_throughput_mbps": float(last_res[vpci_idx]["rx_bps"]),
                     "tx_throughput_mbps": float(last_res[vpci_idx]["tx_bps"]),
    ...
    def collect_kpi(self):
        if not self._queue.empty():
            ...
            if "tx_throughput_mbps" in list(result.keys()):
                if result["tx_throughput_mbps"] < 1.0:
...
if __name__ == '__main__':
    ...
    TREXVNF = TrexTrafficGen(VNFDEN)
    TREXVNF.instantiate(MOCK_SCENARIO, MOCK_CONTEXT)
    TREXVNF.run_traffic(FixedProfile(MOCK_TRAFFIC_PROFILE))
    for index in range(10):
        print(TREXVNF.collect_kpi())
        time.sleep(1)
    TREXVNF.terminate()
