...
class PciAddress(object):
    ...
    def __init__(self, address):
        ...
        PCI_PATTERN_DBDF = re.compile(PciAddress.PCI_PATTERN_STR_DBDF)
        PCI_PATTERN_BDF = re.compile(PciAddress.PCI_PATTERN_STR_BDF)

        match_dbdf = PCI_PATTERN_DBDF.search(address)
        match_bdf = PCI_PATTERN_BDF.search(address)
        if match_dbdf and match_bdf:
            pos_dbdf = match_dbdf.regs[0][0]
            pos_bdf = match_bdf.regs[0][0]
            match = match_bdf if pos_bdf < pos_dbdf else match_dbdf
        elif match_dbdf or match_bdf:
            match = match_dbdf or match_bdf
        else:
